...
| The owner's five calls (item 3) | **(1) withhold the display name until commit; (2) Option 2 left alone — QR stays an in-session verification, not a pre-admission; (3) P9-3-A: make it work — the recommendation is to drop the `!input.appLockEngaged` leg from `presenceState`/`recipeShareState` (the mesh row has none) and retire the input with its projection, re-pinning the 23 040-row product and P7's acceptance clause, because a scoped lock at rest protects private surfaces, not radios; (4) hold the `_fernlet-coach` pair for Coach (§18 decision 4 = its default); (5) the 6 h soak runs tonight, the ladder chosen by its numbers.** (1) and (3) build after the soak's read-out, on the launcher named above. **Built 2026-09-22, BEFORE the read-out — deliberately:** the owner started the build round at midday, the soak was still scheduled for that evening, and the phone already carried the observed build (HEAD's code was byte-identical to `d88062c`'s — every commit since touched `Docs/` only), so the tree could move without touching the soak as long as nothing was re-installed on the phone. (3) = `4d1fa0b` + plan blob `ef3b511`; (1) = `e83ec82`. **Do not install a build from `4d1fa0b` or later on the phone before the soak has run** — that would make the soak's build a different one. | 2026-09-22 (the owner) |
