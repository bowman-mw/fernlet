...
| `init(date:repository:savedRecipeRepository:customItemRepository:coinLedgerRepository:milestoneLedgerRepository:healthKitService:journalNarrativeRepository:foodCatalog:sensitiveVisibilityDefaults:aiAuditLogStore:cookingRunDirectory:photoDocumentsDirectory:proximitySupportDirectory:heartDropKeychainService:)` | Every dependency is defaulted; loads the active repository snapshot, saved recipes, custom items, coin/milestone ledgers, trust vault, retry queue, journal repository, inspector, save hooks, derived signals, and remote reload subscription. The trailing three are per-instance ISOLATION seams (own-photo root, proximity-sidecar root, heart-drop keychain service) — nil means the production path/service; tests pass their own so a wipe in one store cannot reach another's. See `FernletTests/PhotoDirectoryIsolationTests`. |
