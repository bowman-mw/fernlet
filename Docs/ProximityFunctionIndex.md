...
| `MeshKeyAdvertisementReceiveBounds` | `framesPerSenderPerSession` = twice the set's capacity — derived, not chosen: an honest sender re-states its set only when the set changed, and a grow-only set of at most sixteen rows changes at most twice per member (folded, then possibly conflicted). The membership family has no other receive-side limit, and this is its first frame whose per-frame cost can be sixteen Ed25519 verifications. |
...
| `sendKeyAdvertisements(to:)` | Relays this device's whole verified set to NAMED committed peers (non-optional by security, not style: `broadcastMembershipFrame`'s nil path writes every slot, uncommitted included, and this frame is unsealed and enumerates the roster). Re-asserts the self row through `repairOwnKeyAdvertisementIfMissing()` so "once and never again" has a repair path — **below** the roster filter, so a peer's link flap cannot burn all three attempts with nothing owed. The latch is written **only for the fingerprints the write reached** (`writeMembershipFrame`): the earlier form latched every owed peer before the `await`, which turned one lost frame into a session-long `destinationNotAddressable` on a stable pair, because nothing re-arms the latch except the local set changing and no door fires on a fold. Bounded **once per (peer, set version)**: told once, told again only when the set actually grew or gained a conflict mark — not `reGossipedToFingerprints`' spent-forever rule, which would keep a peer from passing on a key it learned in between, and not `askOneReconnectedPeer`'s absence of a bound. Six sites: the three ask doors, the two non-ask membership doors, and the admitter's `grantAdmission(to:meshID:)` — the one link-open no other door covers, since an admission is deliberately not a reconnect. |
...
| `parkKeyAdvertisements(_:outcomes:)` / `foldParkedKeyAdvertisements()` / `clearConflictMarksForNonMembers()` | The receiver-side half the grant door needed (P6 item 1 fix, found by item 2's audit). A row refused **`signerNotAdmitted`** is parked — at most sixteen, one per fingerprint, earliest-wins, memory-only, cleared with the rest of the addressing state — and re-offered through the one fold door at each widening (`attemptLedgerAdoption`, the merge door, the live record insert). Every re-offer is re-decided by that door — folded when it has become provable, dropped and named when it now fails for another reason, re-parked while the signer is still invisible, because a widening arrives in stages (a third device adopts the moment the chain to its OWN admission proves, which can be a two-member ledger). A parked row is raw bytes that have verified nothing and can mark nothing; the work is bounded by the park's capacity and by the ledger's record caps, and a never-named row leaves at the capacity refusal (`mesh.keyAgreement.parkFull`) or the session reset. `signerNotAMember` deliberately never parks — that is a ledger that names the signer and has subtracted it. `clearConflictMarksForNonMembers()` rides the same seams as the relief for the conflict mark's blast radius. |
