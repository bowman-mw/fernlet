...
| `endSessionAfterDiscoveryTimeout()` | Door 3 of `isSessionLive`: the five-minute discovery timeout expired with no committed peer, so the session is over. Raises `sessionSearchGaveUp` BEFORE `stopJoin()`, so `stopSearching()`'s three hooks see an ended session and fire once; refuses while a peer is committed. Does **not** tear the mesh down — the user's own review action does. `startSearching()` clears the flag, so re-arming the radios over the same mesh un-ends it. Replaces the bare `stopJoin()` the app's timeout used to call, which after the fix would have left a pair with no radios, no peer and no review. |
...
| `setSessionOpen(_:)` | Toggles session openness; for closed state, removes uncommitted slots. The **one** writer of `userClosedThisSession`, the sticky local policy `reassertLocallyClosedMode()` consults. |
...
| `handleMeshDescriptor(_:from:)` | Merges, **yields to**, or accepts a descriptor, then sends an admission request if the local member is absent, then re-asserts a closed mode the yield would have replaced. |
