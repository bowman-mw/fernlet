...
| `dispatchMembershipEventPayload(_:plaintext:decoder:slot:)` | Decodes `.meshMemberAdmission` / `.meshMemberDeparture` / `.meshMemberRemoval` / `.meshTerminated` / `.meshInventoryDigest` on a COMMITTED slot, buffers for adoption when this device is still on its bootstrap ledger, else inserts through `MeshMembershipRecordVerifier`, commits durably, then hands the roster move to `applyRosterMove(_:from:)`. |
...
| `foldEpochHeads(_:)` | Seals a reconnecting peer's heads into `epochHeads` through the single `persistSessionContext(addingEpochHead:)` writer. Same-counter divergent heads **coexist**; an overflow past the cap of 8 is named in `droppedEpochHeadCount` (plan §21.3), never silently truncated. |
