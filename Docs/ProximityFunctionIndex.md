...
| `sendRoutedBulk(_:to:now:)` / `sendRoutedDrainBatch(_:to:now:)` | **P5 item 8 extracted the sender.** `sendRoutedBulk` charges the peer's session frame budget as its **first statement, before its first `await`** — a pump delivers 64 frames synchronously, so a charge after the sends would double-spend — then sends manifests, chunks, receipts. It has exactly two call sites, the drain answer and the departure push, and logs nothing: each caller keeps its own audit vocabulary, so a drain answer and a hand-off push are never confused in a transcript. A batch that no longer fits is refused whole, never part-served. |
