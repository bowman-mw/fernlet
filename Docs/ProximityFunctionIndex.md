...
| `applyRunState(links:discovery:)` | **P7 item 3, pass A**: the policy's `apply(_:)` seam for the two mesh radios, taking BOTH resolved directives at once because `startJoin()` / `stopJoin()` is one door for both. Ensures searching through `FriendsDiscoveryEntry`'s three-way; stands down only when both directives are `stop` AND no peer is committed — `ContentView.stopFriendsDiscovery()`'s bail, preserved, because `stopSearching()` empties the slots and clears the group-key state and so would END a live mesh. The P8-only combination (links `run`, discovery `stop`) moves nothing and logs `noStandAloneDiscoveryStop`. **P7 item 5 pass 2 added a second `held` row**: a `.fresh` entry over an UNANSWERED launch-restore offer holds and logs `resumeOffered`, because `startJoin()` resets the session state machine and would clear `offersForegroundResume` and `restoredSessionContext` before the Friends card could be drawn. Full table and reasoning in the run-state-seams section below. |
| `resumeSearchingForPartitionedMesh()` | **`startJoin()` minus every reset** (P6 item 2): re-arms the radios for a session whose founded mesh outlived its links, keeping the mesh, the ledger, the ceiling, the advertisement set, the photos, the film quota and the state machine. `FriendsDiscoveryEntry.resume` is its one shipping caller, reached from `armFriendRadios()` inside `applyRunState(links:discovery:)` since P7 item 3's pass B retired `ContentView.startFriendsDiscovery()`. Without it a founded pair that blipped and then left the tab had no way back in — `startSearching()` is private and the `!isInSession` guard passes over a mesh that outlived its link — and `startJoin()` is not the answer: it would nil the ceiling on a mesh that can never re-found. Its real predecessor is `stopSearching()`, not `startJoin()`: the group key, the epoch keyring and the rotation/beacon timers are already gone, so the resumed session is keyless until the heal re-keys it (asserted). `isProximityJoin` is RESTORED from `sessionEnteredByProximityJoin`, never asserted — a launch-restored mesh reaches this arm and must not be flipped into proximity-join mode. Refuses a terminal `sessionState` outright. |
| `endSessionAfterDiscoveryTimeout()` | Door 3 of `isSessionLive`: the five-minute give-up interval expired with no committed peer, so the session is over. Raises `sessionSearchGaveUp` BEFORE `stopJoin()`, so `stopSearching()`'s three hooks see an ended session and fire once; refuses while a peer is committed. Does **not** tear the mesh down — the user's own review action does. `startSearching()` clears the flag, so re-arming the radios over the same mesh un-ends it. **Three arms, all inside this module since P7 item 3's pass B** (fix review P2-1, plus the pass-A obligation): `armSessionGiveUpClock(now:)` at the two slot-loss doors and on `armFriendRadios()`'s `.resume` row, and `armFreshSearchGiveUpClock(now:)` on its `.fresh` row — the successor to the retired `ContentView.armDiscoveryTimeout()`, which the app could not keep once `startFriendsDiscovery()` went. All three schedule ONE deadline through `scheduleSessionGiveUp(now:)`, cancelled by `noteCommitIntoMesh(peer:)`, by `stopSearching()` and by every teardown, with `evaluateSessionGiveUp(now:)` reading it. |
| `armFreshSearchGiveUpClock(now:)` | **P7 item 3, pass B**: door 3's arm for a FRESH, peerless search — `FriendsDiscoveryEntry.fresh`, i.e. no mesh and no committed slot. A second arming door rather than a widened guard, because `armSessionGiveUpClock(now:)` refuses `currentMesh == nil` by name (the ledgerless pairwise shape ends at slot loss, door 4) and would also refuse on `isSessionLive`, which a fresh row makes false — so both of its terms reject exactly the row this one exists for. Its whole precondition is `!hasCommittedPeer`. Same clock, same `discoveryGiveUpInterval`, same on-fire effect (`endSessionAfterDiscoveryTimeout()`), so a fresh search and a blip cannot drift to different timeouts. |
| `scheduleSessionGiveUp(now:)` | The two arming doors' shared body: cancels any standing `sessionGiveUpTask`, replaces `sessionGiveUpDeadline` with `now + discoveryGiveUpInterval`, logs `mesh.session.giveUpClockArmed` and starts the one `// host-pin: timer` task whose wake calls `evaluateSessionGiveUp(now:)`. Split out so each arming door is only its own guard — one clock, one interval, one task marker. Idempotent: re-arming replaces the deadline rather than stacking a second task. |
| `leaveMesh()` | Clears mesh/admission/removal/group-key state, resets quotas, and stops discovery. |
| `proposeRemoval(of:)` | Starts removal voting for a non-local participant, or leaves directly for the last pairwise peer. The shortcut depends on `sessionParticipants` being a union (P6 item 2): a roster of two can never reach §10.4's quorum, so "remove the only other person" = "end the session" is the whole user-facing answer. |
...
| `onSlotConnected(at:identity:)` | Handles every slot commit: the session-roster capture, the shop formation hook, the **founding at the FIRST commit** when this is a proximity-join session with no mesh (P6 item 2), else the descriptor send — then the vouch list and the shared per-commit tail. |
...
| `unwindNewbornMesh()` | The COMPLETE unwind of a founding given up — `abandonUnpersistedSession()` plus `clearGroupKeyState()` and `clearKeyAdvertisementState()`. The group key is load-bearing: a yielder past one rotation would be refused `droppedStaleEpoch` by the grant it then needs and could never join. It carries `lastRotationBlockReason` ACROSS the unwind (P6 item 2 pass B): `clearGroupKeyState()` nils it, and on the founding-failure path that string is the one surface saying why the founding was abandoned — the diagnosis has to outlive the state it diagnoses. **Residual CLOSED at P7 item 4 pass B:** the unwind still nils `sessionCeiling`, and for three phases nothing re-armed it — so a yielder ended `.idle` with no ceiling, which stopped being latent the moment item 4 gave `enforceSessionCeiling` its poller (a poller can only enforce a ceiling that was ARMED, so every tick returned nil for exactly that device). `handleMeshDescriptor`'s yield arm now calls `adoptSessionCeiling(of:now:)` right after it assigns the winner's descriptor, deriving `createdAt + 6 h` from the ADOPTED mesh — the winner's instant, never a fresh six hours. Still open and narrower: an ordinary proximity JOINER reaches `currentMesh = incoming` down the other arm of that door and arms no ceiling. |
...
| `yieldsNewbornMesh(_:to:from:)` | P6 item 2's repair for the double mint founding at one commit makes reachable: adopt the incoming mesh iff a different meshID, the local mesh is one this device founded and is ALONE in (one member, itself, derived roster ≤ 1 — a fresh joiner's bootstrap roster is also 1, so the descriptor is what separates them), the routed index is provably empty, the sender is this device's only committed peer, and either the incoming mesh has more than one member or `foundsPairwiseMesh` says so. Audits `mesh.descriptor.yieldedNewbornMesh`. |
| `holdsNoRoutedItem()` | Whether the routed store provably holds nothing — fail-closed on every non-`loaded` state but `.absent`. |
| `mergeMeshDescriptor(_:incoming:)` | Merges descriptor name/mode by timestamp and appends non-removed new members. A foreign meshID is dropped by name (`mesh.descriptor.droppedForeignMesh`, R7 — newly reachable and formerly silent). |
...
| `adoptSessionCeiling(of:now:)` | **P7 item 4 pass B.** The third arming door, for the device that ADOPTS somebody else's mesh: `hardDeadline` is the adopted descriptor's `createdAt + 6 h` (a ceiling is a property of the MESH — two members that disagreed about when it ends would be two sessions) and `startedAt` is this run's instant, so the monotonic budget is what is LEFT. Guarded on `sessionCeiling == nil`, so a re-broadcast descriptor cannot restart the budget. Called from `handleMeshDescriptor`'s yield arm, which is what closed the yielding founder's no-ceiling gap. |
| `enforceSessionCeiling(now:monotonicElapsed:)` | At either bound: mark terminated, persist, **await** `terminated.v1`, end the session. A refused save abandons the emit. **`public` since P7 item 4, and first of the three calls the app's poller makes** — `ProximityRunPolicyHost`'s 30-second tick, armed on the rise of `isSessionLive`. It passes `monotonicElapsed: nil`, so the elapsed runtime is measured here from `sessionMonotonicOrigin`: the app owns no session clock, and that `ContinuousClock` origin is the same monotonic-by-construction number P8's continued-task progress bar reads (plan §2672–2679). |
| `evaluateIdleLapse(now:)` | Plan §8.2's 30-minute window as a value, evaluated on demand — no timer, nothing to spin. **`public` since P7 item 4**, second of the poller's three calls: after the ceiling, which may already have ended the session, and before the partition evaluation, which is what ARMS the window this one reads. |
| `evaluatePartition(now:)` / `evaluatePartition(reachable:now:)` | Plan §10.2's partition detection, evaluated **on demand** in the same idiom — no timer of its own, because **P7 item 4 built the ONE poller that drives all three** and it lives in the app target. Re-derives `branchView`, raises `linksLost` / `linksRestored`, and **mints nothing**: the derived roster does not move. Only the `now:` convenience is `public`: the reachable set is the live transport's answer, so the app supplies an instant and never an opinion about who is present. |
...
| `lastRejoinBarHit` | **P7 item 5, pass 2 fix review (P1-3).** The bar as a HIT THIS RUN, never the durable `rejoinBar`: the reason of the last entry `rejoinRefusal(for:)` actually REFUSED, or nil while nothing has been refused this run. Set only at the three doors that enforce the durable bar against this device — a barred `acceptForegroundResume(now:)`, a dropped mesh descriptor (`handleMeshDescriptor(_:from:)`), a dropped admission grant (`handleAdmissionGrant(_:slot:senderSigningPublicKey:)`) — and cleared by `resetSessionStateMachine(keepingTerminalState:)` (every founding, the newborn-mesh yield's unwind, `startJoin()`) and by a successful accept, so a session that really starts takes the notice down with it. Run-scoped and, unlike the four `@ObservationIgnored` restore properties beside it, deliberately observed: a hit happens while a surface is on screen and is the one fact here a view must repaint for in the same turn it is recorded. Published by `sessionResumeProjection`. |
| `sessionResumeProjection` | **P7 item 5, pass 2 — the ONE thing the launch restore exports.** A read-only computed `MeshSessionResumeProjection`: the flattened outcome kind (`MeshSessionResumeProjection.Outcome`, eight frozen tokens that are the app's `ProximityRestoreOutcomeKind` spellings one for one), `offersForegroundResume`, and `rejoinBarHit` — **a rejoin-bar HIT this run**, straight off `lastRejoinBarHit`, and never the bar's reason matched against the mesh currently held. **Since the pass 2 fix review (P1-3)**, this replaced `rejoinRefusal(for: restoredSessionContext?.meshID ?? currentMesh?.meshID)` — the durable bar matched against the mesh in hand: that bar is the durable half of plan §8.2 and is cleared NOWHERE (`resetSessionStateMachine(keepingTerminalState:)` says so by name), and a `terminated` context is never reaped while an `expired` one is written back AS terminated, so at launch the mesh in hand IS the mesh the bar names and matching still re-presented "that session has ended" on every cold start until some new session overwrote the sealed file. A `terminated`/`expired` launch is now silent (both carry `offersForegroundResume == false`) until a door refuses an entry. Payloads are dropped because none of them changes what is presented, and `ProximityResumeDecisionTests` enumerates every payload variant through the mapping to say so. `offersForegroundResume` and `rejoinBarHit` are deliberately observed, unlike `lastSessionRestoreOutcome` and the durable `rejoinBar` behind them (both stay `@ObservationIgnored`): a surface must repaint the turn a deferred-then-retried restore raises an offer, an accept spends it, or a refused try raises a hit — not only on the accept that moves `currentMesh`. |
| `acceptForegroundResume(now:)` | **P7 item 5, pass 2 — the door P6 item 7 left un-built.** Adopts `restoredSessionContext` into `currentMesh`, pins `sessionQuotaMeshID` to that mesh (so an adoption cannot read as a new session and hand back a free film quota), arms the ceiling from the context's own `hardDeadline` if the restore has not already (guarded on `sessionCeiling == nil`, so a resume after a retried restore is idempotent rather than a second monotonic origin), clears the offer and audits `mesh.sessionResume.accepted`. **Returns `MeshForegroundResumeOutcome`, not `Bool`** (pass 2 fix review, P1-3d): `.accepted` when adopted, or `.refused(Reason)` naming which guard closed — a caller that only knew `false` could not tell a barred try from an idempotent second tap. **It arms no radio** — no `startJoin()`, no `startSearching()`, not even `updateDiscoveryInfo()` — which is why it is not a needle for P7 item 3's zero wall. Its point is the Friends three-way: without the adoption `isInSession` stays false after a relaunch, `FriendsDiscoveryEntry.entry(isInSession:hasCommittedPeer:)` answers `.fresh`, and the first Friends visit calls `startJoin()` — founding a SECOND mesh beside the one on the disk. With it the entry is `.resume`, into `resumeSearchingForPartitionedMesh()`, whose own doc already names this caller. The adopted descriptor is the weakest possible claim (`MeshSessionContext` carries no name and no mode, so both are stamped `.distantPast` and any gossiped descriptor wins them under `mergeMeshDescriptor`'s last-write-wins; `meshID` and `createdAt` come off the sealed file). Fail-closed with five frozen `Reason` cases, each with its own frozen `token` for the unchanged `mesh.sessionResume.refused` audit line: `noOffer`, `noContext`, `meshHeld`, `rejoinBarred(MeshSessionTerminationReason)` — the one refusal a surface presents, and what raises `lastRejoinBarHit` — and `sessionEnded`. |
| `declineForegroundResume()` | **P7 item 5, pass 2.** Clears `offersForegroundResume` and audits `mesh.sessionResume.declined`, so the offer is not re-presented on the next visit to the Friends tab. It adopts nothing and keeps `restoredSessionContext`, the restored ledger and the restored key advertisements — a declined resume must still let an expiry found at launch be written back and a custodied routed item drain the moment a link forms. **Silent when no offer stands**, which is what lets the app's single `onDismiss` closure serve the two NOTICES as well without minting a decline nobody made. |
| `restoredMeshDescriptor(from:)` (private) | The descriptor a resumed session adopts: this device's own member row, the sealed context's `meshID` and `createdAt`, a generated name and this device's session mode — both stamped `.distantPast` so the merge gives them away to the first real descriptor that arrives. The member list is this device alone, exactly as `promoteToMesh()` mints one, because the descriptor is GOSSIP and the restored `MeshMembershipLedger` is the membership truth. |
...
| The frozen audit vocabulary | Pass A: `mesh.keyAgreement.folded` / `…conflicted` / `…setFull` / `…rejected` (an honest replay audits nothing). Pass B adds `mesh.keyAgreement.notDurable` (a fold the store refused, rolled back out of memory — with its VERSION, third review P3 5, so nobody is re-told a set they hold), `mesh.keyAgreement.senderBudgetSpent` (the per-sender frame bound, or the map's own roster-cap growth) and `mesh.sessionRestore.keyAdvertisementsRestored` (counts only: kept and dropped). **The park's six** (second fix review; rowed here by the third review's P3 10, which found two of them in no index and no DocC page): `mesh.keyAgreement.parked` (a refused row took a slot in its authenticated sender's share), `…parkedReoffered` (a widening drained the park — count only), `…parkCollision` (two different rows for one member inside one share, with `kept: newcomer` or `kept: held` as the only thing an operator could not infer), `…parkFull` (the share or the sender bound is spent), `…parkDropped` (a row that failed `failedRosterMovesPerRow` verified roster MOVES is evicted rather than left immortal) and `…parkRolledBack` (a re-offer whose seal the store refused put the park back with the set). English forever, and **counts only — never a fingerprint**, which is why the member each decision is about rides the outcome value and not the log line. |
...
| `sweepingExpired(now:)` / `sweepingOrphanChunkFiles()` / `dropping(item:reason:)` | On demand, never on a timer — and P7 item 4's poller does **not** change that: the one timer it built drives the three SESSION judgements (ceiling, idle lapse, partition) and reaches nothing in the routed store, so these three keep their existing seams (the drain answer, the next session's ledger arm, `startSearching()` while a hold is showing). The orphan sweep's loop is bounded by **2 ×** `maxHeldChunkFiles` and reports `sweptToCeiling`, because a directory that got over the cap is exactly what orphans produce. |
...
### `ProximityRunState.swift`, `MeshNetworkManager.swift`, `PresenceManager.swift`, `ProximityRecipeShareManager.swift` — the run-state seams (P7 item 3, pass A)

Plan §13's `apply(_:)` doors, given to the managers so P7 item 3's pass B can make
`ProximityRunPolicyHost` their single writer. **The policy stays in the app target** (§13 option A);
only the DIRECTIVE crossed the module boundary, because a package target cannot name an app type and
two copies of one vocabulary is the shape that drifts. The host calls each seam with
`ProximityRunDecision.isUp(_:)` already applied, so a seam sees `run` or `stop`; `foregroundOnly` is a
policy/UI answer about two scene phases and a manager knows about neither. A `stop` is **stand down**,
never "end the session" — teardown is `tearsDownSession`'s, and no door here ends a mesh.

| Type / Function | What It Does |
| --- | --- |
| `ProximityRunState` | `run` / `foregroundOnly` / `stop`, `CaseIterable` with **frozen** `String` `rawValue`s (they are what the seams log as `state=…`, so a rename breaks somebody's saved trace). Moved down from `App/Fernlet/ProximityRunPolicy.swift` in this pass; the policy and the other four app-side vocabulary types stayed. |
| `ProximityRunState.isUp(inForeground:)` | The directive resolved against the ONE foreground fact (`FernletApp.routedGateForeground(for:)`'s answer): `run` is up in both phases, `stop` in neither, `foregroundOnly` follows the scene. The six-row table is pinned literally. |
| `ProximityRunStateSeam` (internal) | The seams' frozen English vocabulary in one place: `mesh.runState.applied` (a CHANGE — a door pushed twice with the same value logs once), `mesh.runState.held` + `reason` (a directive that deliberately moved nothing), `mesh.runState.unresolved` (one that arrived unresolved), the four radio names — spelled identically to the app's `ProximityRadio` `rawValue`s, which this module may not import, and held against them by a cell — the pair token `meshLinks+discoveryAdmission`, and the two refusal reasons `committedPeer` / `noStandAloneDiscoveryStop`. |
| `ProximityRunStateSeam.isUp(_:radio:)` | The one resolver. A seam handed `foregroundOnly` treats it as `run` — the fail-SAFE direction, since standing a radio down on an unresolved directive could end a live mesh — and logs `unresolved`, so the claim is a cell rather than a comment. |
| `MeshNetworkManager.applyRunState(links:discovery:)` | **One door for two radios, because `startJoin()` / `stopJoin()` already is one.** `discovery == run` ⇒ ensure searching, idempotent on `isSearching` and resolved through `FriendsDiscoveryEntry` (`fresh` ⇒ `startJoin()` + `armFreshSearchGiveUpClock(now:)`, `resume` ⇒ `resumeSearchingForPartitionedMesh()` + `armSessionGiveUpClock(now:)`, `none` ⇒ nothing) — never a fourth copy of that three-way, and since P7 item 3's pass B the two arms reproduce `FriendsDiscoveryEntry.armsDiscoveryTimeout`'s table with the door each row can use, which is where the retired `ContentView.armDiscoveryTimeout()` went (P7 item 4 still owns the separate session poller). `links == stop` **and** `discovery == stop` ⇒ stand down, **unless a peer is committed**: `ContentView.stopFriendsDiscovery()`'s bail is preserved exactly, because `stopSearching()` empties `slots`, drops `slotTrustPolicies`, cancels every slot coordinator and runs `clearGroupKeyState()` — running it over a committed slot ENDS a live mesh, and a background or tab exit is the OS suspending the links (`.linksLost` → partition → restore). The guard is `hasCommittedPeer`, never `isSessionLive` (projections and ceremonies) and never `isInSession` (the layout swap). The **P8-only** combination — links `run`, discovery `stop` — moves nothing and logs `noStandAloneDiscoveryStop`: no primitive stops browsing and advertising while KEEPING the committed links, and inventing one is P8's work, not this pass's. **A `.fresh` entry over an unanswered launch-restore offer** (P7 item 5 pass 2) is the second `held` row: `startJoin()` ends in `resetSessionStateMachine(keepingTerminalState: false)`, which clears `offersForegroundResume`, `restoredSessionContext` and the ceiling — so arming it would wipe the restore before its Friends-tab affordance existed, and the offer's own sentence ("It isn't looking for anyone until you say so") would be false at the instant it appeared. The row logs `resumeOffered`, and the hold is bounded by the user's ANSWER rather than by a clock: both of the card's actions clear the offer and the app re-pushes the policy after either. Idempotent, bounded, and it starts no `Task`, arms no timer and registers no observer. |
| `PresenceManager.applyRunState(_:)` | `run` ⇒ `start()` if not already running, `stop` ⇒ `stop()` if it is. Both underlying doors already guard on `isRunning`, so the guards here are about the AUDIT: `applied` names a change, so a repeated push logs once. Consent-off, a wipe and a below-age verdict reach this radio as policy INPUTS producing `stop`, never as second owners reaching around the policy. |
| `ProximityRecipeShareManager.applyRunState(_:)` | The twin, over the recipe listener's own run flag, with the same three lines and the same contract. |
...
| `MeshSessionResumeProjection` | **P7 item 5, pass 2 — `public`, and the ONLY thing about the restore that crosses the module wall.** Three facts: `outcome` (the nested `Outcome`, eight frozen tokens identical to the app's `ProximityRestoreOutcomeKind` `rawValue`s, with `notAttempted` standing for the `nil` outcome), `offersForegroundResume`, and `rejoinBarHit` — **a rejoin-bar HIT this run** (`MeshNetworkManager.lastRejoinBarHit`: the reason of the last entry `rejoinRefusal(for:)` actually REFUSED), never `rejoinRefusal(for:)`'s answer matched against the mesh in hand and never the global `rejoinBar?.reason`. **Since the pass 2 fix review (P1-3)**: matching the durable bar against the mesh in hand did not close the defect either, because at launch the mesh in hand IS the mesh the bar names — that bar is cleared nowhere, so it would still name the SAME mesh ended on every cold start for the rest of the install until some new session overwrote the sealed file; the HIT crosses instead, silent until a door refuses an entry. `Outcome.init(restoring:)` is the internal flattening from `MeshSessionRestoreOutcome` — exhaustive, so an eighth outcome case is a build error — and payloads are dropped because a deferral's reason, a refusal's cause and a corruption's detail change what is LOGGED, never what is presented. |
| `MeshForegroundResumeOutcome` | **P7 item 5, pass 2 fix review — `public`, what `MeshNetworkManager.acceptForegroundResume(now:)` returns instead of `Bool`.** `.accepted` — the restored context was adopted, `currentMesh` now names it, no radio armed — or `.refused(Reason)`, naming which of five guards closed. `Reason`: `noOffer`, `noContext`, `meshHeld`, `rejoinBarred(MeshSessionTerminationReason)` — **the one refusal a surface presents**, the user just tried to re-enter a mesh that ended, and the one that raises `MeshNetworkManager.lastRejoinBarHit` — and `sessionEnded`. Each case carries a frozen `token` (`Reason.token`) for the unchanged `mesh.sessionResume.refused` audit line — English forever, never display copy. A caller that only knew `Bool` could not tell a barred try from an idempotent second tap. |
| `MeshSessionTerminationReason` (now `public`) | **P7 item 5, pass 2 made it `public` and changed nothing else** — still `nonisolated`, `Sendable`, `CaseIterable`, still eight frozen at-rest tokens, and `endsTheMeshForEveryone` stays internal because no app surface asks that question. It is the one of the restore's three types that crossed, so `MeshSessionResumeProjection` can carry the reason as a CASE rather than as a `String` for the app to re-parse; `MeshSessionRestoreOutcome` and `MeshSessionRejoinBar` did not cross, and a surface has no business holding a mesh id. Declared in `MeshSessionContext.swift`. |
...
| `applyRunState(_:)` | **P7 item 3, pass A**: the policy's `apply(_:)` seam for this listener — `run` ⇒ `start()` if not running, `stop` ⇒ `stop()` if it is, idempotent, one `mesh.runState.applied` line per CHANGE. The `allowNearbyRecipeShares` toggle and the wipe reach it as policy INPUTS producing `stop` from pass B on. |
...
| `applyRunState(_:)` | **P7 item 3, pass A**: the policy's `apply(_:)` seam for this radio — `run` ⇒ `start()` if not running, `stop` ⇒ `stop()` if it is, idempotent, one `mesh.runState.applied` line per CHANGE. Lifecycle ownership moves from `ContentView`'s scene/tab/lock chain to `ProximityRunPolicyHost` in pass B. |
