...
| `MeshSessionResumeProjection` | **P7 item 5, pass 2 — `public`, and the ONLY thing about the restore that crosses the module wall.** Three facts: `outcome` (the nested `Outcome`, eight frozen tokens identical to the app's `ProximityRestoreOutcomeKind` `rawValue`s, with `notAttempted` standing for the `nil` outcome), `offersForegroundResume`, and `rejoinBarReason` — which is `rejoinRefusal(for:)`'s answer for the mesh in hand and never the global `rejoinBar?.reason`, because that bar is cleared nowhere and would name the NEXT mesh ended with the last one's reason. `Outcome.init(restoring:)` is the internal flattening from `MeshSessionRestoreOutcome` — exhaustive, so an eighth outcome case is a build error — and payloads are dropped because a deferral's reason, a refusal's cause and a corruption's detail change what is LOGGED, never what is presented. |
