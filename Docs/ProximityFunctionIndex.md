...
| `hasCommittedPeer` | Whether any slot holds a committed fingerprint — the **lifecycle** half (P6 item 2). The three session-end hooks, `ConnectView.presentDisconnectReviewIfNeeded()` and the app's two radio-stand-down guards read this; on `isInSession` they would never fire again once a pair founds a mesh at its first commit. |
...
| `promoteToMesh()` | Mints this device's mesh descriptor at the FIRST proximity commit and **founds** it through `foundMesh(_:now:)`; returns `false` when the context could not be sealed, in which case nothing is announced (P6 item 2). |
...
| `unwindNewbornMesh()` | The COMPLETE unwind of a founding given up — `abandonUnpersistedSession()` plus `clearGroupKeyState()` and `clearKeyAdvertisementState()`. The group key is load-bearing: a yielder past one rotation would be refused `droppedStaleEpoch` by the grant it then needs and could never join. |
...
| `handleMeshDescriptor(_:from:)` | Merges, **yields to**, or accepts a descriptor, then sends an admission request if the local member is absent. |
