...
| `MeshKeyAdvertisementPark` / `MeshParkedKeyAdvertisement` / `MeshParkedKeyAdvertisementOffer` / `MeshKeyAdvertisementParkOutcome` / `MeshKeyAdvertisementParkBounds` | The side container for rows refused **`signerNotAdmitted`**, per AUTHENTICATED SENDER (P6 item 1, second fix review). The first shape was one flat map keyed by member, first-arrival wins, capped at the set's 16 — squattable by a single misbehaving *member*, because `signerNotAdmitted` is answered BEFORE the signature check and one frame carries sixteen rows: one frame disabled the park for the session, and junk sitting in member M's slot meant M's genuine relayed row was silently not parked. Three named bounds: `rowsPerSender` (= the set's capacity — a whole frame's worth, which is exactly what the joiner-bootstrap case needs, since it can prove none of what its admitter relays), `senders` (= the roster cap, R3) and `failedWideningsPerRow` (3 — not one, because a widening arrives in stages; it is what keeps a bigger container cheap, since a row costs at most three verifications in its whole lifetime). Within a share **first parked wins** — arrival order, never the set's `precedes` rule, which orders on an unverified row's attacker-chosen `advertisedAt` — and a row that has already failed a widening yields its slot to a newcomer, audited either way (`mesh.keyAgreement.parkCollision`, `…parkFull`, `…parkDropped`). Nothing in here is verified and nothing in here can mark anything. Memory-only, no wipe row owed. |
