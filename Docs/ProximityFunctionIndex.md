...
| `MeshRoutedCanonicalStore` | The frozen slot `friendPhotoWall` / `sessionTranscript` / `heartLedger` — a token, never a closure and never a store type, so this file stays `nonisolated`, clock-free and store-free. **Read by P6's dispatch, by nothing today**, and said so rather than dressed up. |
