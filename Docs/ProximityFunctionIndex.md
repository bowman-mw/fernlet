...
| `onPeerDisplayNameDisclosed` | Subscriber seam for the late name; `MeshNetworkManager` RENAMES the seated roster entry (`renameSessionParticipant(fingerprint:to:)` — never appends, so a refused stranger's named frame in the teardown window mints no row) and `ProximityRecipeShareManager` re-files the picker row (keeping its advertised label until the signed name arrives). |
...
| `isSessionLive` | **The ONE predicate "has the session ended" is the negation of** (P6 item 2 fix). `currentMesh != nil ? (!sessionState.hasEnded && !sessionSearchGaveUp) : hasCommittedPeer` — so for a mesh-holding session, session end means MESH end. Four doors, and no others: `leaveMesh()`/End Session (the mesh is nil and the slots are gone by the time `stopSearching()` runs the hooks), any terminal `sessionState` (every terminal edge carries `.stopParticipation`, and the state is assigned BEFORE the effects run), `endSessionAfterDiscoveryTimeout()` (whose clock the MANAGER arms at the slot-loss doors, `sessionGiveUpDeadline` — the tab's arm is single-shot per visit), and slot loss while `currentMesh == nil` (the legacy pairwise session). Read by the three session-end hooks and by `ConnectView.presentDisconnectReviewIfNeeded()`; P6 item 4's live-transcript projection reads it too, so a message completing during a blip is neither dropped nor marked final against a transcript that was never cleared. It is **reversible** (`startSearching()` clears the give-up flag) while the transcript clear is not, which is why item 4's projection ANDs it with the item's mesh and with `transcriptGeneration`. |
...
| `sessionParticipants` | Builds a de-duplicated participant list from local identity **∪** current mesh members **∪** committed slots, excluding removed members (P6 item 2: a union, not a choice — a freshly founded descriptor holds one member, so the `if let mesh` branch alone hid the committed peer for the whole admission round trip and made `proposeRemoval`'s pairwise shortcut unreachable). Members are listed first, so the dedup keeps the identity display name over the slot's transport hint. |
| `leaveSession()` | Clears session photo metadata and leaves the mesh/pairwise session. |
| `leaveSessionAfterNotifyingPeers()` / `leaveSessionAfterNotifyingPeers(clock:)` | Ends the session and tears down transport. **No longer sends `.sessionGoodbye`** (P3 item 3): the legacy frame is parsed, never emitted. Since P4 item 6 (plan §10.6) the ending is `MeshDevelopmentPlan`'s: a merged roster larger than two departs with the custodians the branch view says are reachable, a genuine final pair signs the termination. The `clock:` form injects the instant the 15-second handoff window opens and the instant it closes, so the bound is asserted on a test clock rather than by timing a real send. **A termination holds the radio up until the partner closes its end** (2026-09-22): `awaitPartnerReceiptOfTermination(_:)` asks `MeshTransportSession.awaitRemoteClose(of:within:)` to wait (bounded, `terminationReceiptGraceSeconds` = 1.5 s, the radio's own ceiling 2 s) on the committed slots of the other **signed-roster members** before `.terminationSent` stops participation — the partner's own acknowledgement that it read the record. The answer is a `MeshRemoteCloseOutcome` (`closed` / `boundReached` / `nothingToWaitFor` / `cancelled`), audited as `mesh.development.partnerReceipt` — `nothingToWaitFor` with `partners=0` when no committed partner was there to watch, rather than silence. `closed` means the link ended, not that the partner read the record: the radio cannot tell the partner's close from a local link failure. A bounded best effort, not a delivery guarantee: a suspended partner ends by its own five-minute give-up. A departure does not wait. **One leave at a time:** a call while `sessionState == .handingOff` returns at once, audited `mesh.development.alreadyInFlight` — the pairwise "Ask to remove" now leaves asynchronously, so a second tap would otherwise sign a second record and repeat the transfer and the wait. While handing off, a lost link raises no `linksLost` and arms no give-up clock. |
...
| `resumeSearchingForPartitionedMesh()` | **`startJoin()` minus every reset** (P6 item 2): re-arms the radios for a session whose founded mesh outlived its links, keeping the mesh, the ledger, the ceiling, the advertisement set, the photos, the film quota and the state machine. `FriendsDiscoveryEntry.resume` (from `ContentView.startFriendsDiscovery()`) is its one shipping caller. Without it a founded pair that blipped and then left the tab had no way back in — `startSearching()` is private and the `!isInSession` guard passes over a mesh that outlived its link — and `startJoin()` is not the answer: it would nil the ceiling on a mesh that can never re-found. Its real predecessor is `stopSearching()`, not `startJoin()`: the group key, the epoch keyring and the rotation/beacon timers are already gone, so the resumed session is keyless until the heal re-keys it (asserted). `isProximityJoin` is RESTORED from `sessionEnteredByProximityJoin`, never asserted — a launch-restored mesh reaches this arm and must not be flipped into proximity-join mode. Refuses a terminal `sessionState` outright. |
| `endSessionAfterDiscoveryTimeout()` | Door 3 of `isSessionLive`: the five-minute give-up interval expired with no committed peer, so the session is over. Raises `sessionSearchGaveUp` BEFORE `stopJoin()`, so `stopSearching()`'s three hooks see an ended session and fire once; refuses while a peer is committed. Does **not** tear the mesh down — the user's own review action does. `startSearching()` clears the flag, so re-arming the radios over the same mesh un-ends it. **Two arms, and the manager's is the load-bearing one** (fix review P2-1): `armSessionGiveUpClock(now:)` at the two slot-loss doors, cancelled by `noteCommitIntoMesh(peer:)` and every teardown, with `evaluateSessionGiveUp(now:)` reading the deadline; `ContentView.armDiscoveryTimeout()` keeps the case the tab owns (a fresh search that never had a peer), because the app's arm is single-shot per visit and gave a pair that blipped six minutes in no door 3 at all. |
| `leaveMesh()` | Clears mesh/admission/removal/group-key state, resets quotas, and stops discovery. |
| `proposeRemoval(of:)` | Starts removal voting for a non-local participant, or — for the last pairwise peer — leaves through `leaveSessionAfterNotifyingPeers()` (2026-09-22; it used to call the silent `leaveSession()`, which ended the session on this phone only and left the partner in a mesh of one). The shortcut records the peer in `askedToRemoveFingerprint`, which `recordSessionParticipant` refuses until the session's teardown (`leaveMesh()`, the same synchronous step that removes the slots and promotes the roster) or the next `startJoin()`: the leave is asynchronous, and a re-dial committed inside it (a re-seat or a dwell) otherwise re-recorded the peer onto the keep-as-friend prompt. Deliberately not `removedMemberFingerprints`, which also feeds key distribution, rotation recipients and admission. The shortcut depends on `sessionParticipants` being a union (P6 item 2): a roster of two can never reach §10.4's quorum, so "remove the only other person" = "end the session" is the whole user-facing answer. |
...
| `setMeshMode(_:)` | Changes open/closed mode, updates discovery info, and broadcasts descriptor. The stamp is **monotonic in the descriptor it replaces** (`max(now, held + 1 ms)`), not a bare `Date()` (P6 item 2 fix): the merge resolves `mode` by LWW on `modeSetAt` and an adopted stamp comes from the PEER's clock (clamped only from above, `+60 s`), so a device answering a peer whose clock is ahead stamped its own change in the past and every other member's merge door discarded it. |
| `setSessionOpen(_:)` | Toggles session openness; for closed state, removes uncommitted slots. The one writer of **`true`** for `userClosedThisSession`, the sticky local policy `reassertLocallyClosedMode()` consults — cleared by the same control on re-open and by each session boundary (`startJoin()`, `leaveMesh()`): three writers, one of which can say yes (fix review P3-6). |
| `reassertLocallyClosedMode()` | Re-applies this device's user's own **closed** choice after every descriptor — the yield it was written for (pass B, P2-5) and every later merge (its fix, P2-3). Keyed on `userClosedThisSession`, because a monotonic stamp cannot survive the peer's NEXT descriptor, which is honestly later still: the choice is a local policy, not a bid in a clock race. Consequence, deliberate and fail-closed: a mesh stays closed while any member's user has closed it. Terminates — the guard is `mode == .open`. |
| `addPhoto(_:)` / `shareRoutedPhoto(itemID:addedAt:imageData:session:)` | Enforces the 10-shot quota, resizes, echoes to this device's own wall **unconditionally** (D-13.8 — both retired send arms cached before any send, so a solo member has always got a wall entry and no error), then frames the bytes as a `MeshRoutedPhotoBody` and hands them to the routed sender door. **P5 item 13 replaced the transport, not the capture:** the group-key seal and the epoch-0 plaintext broadcast are gone, the destination set is the full roster at creation rather than whichever slots were active, and only a mint that was ATTEMPTED and failed reaches the user (as the frozen `MeshRoutedShareRefusal` token on the observed `routedShareRefusal`, through `noteRoutedShareRefusal(_:error:)`; the app forks it into localized copy via `RoutedShareRefusalCopy` — no sentence is composed in the package). **P8 item 0 (c):** the door's third answer is no longer silent — `.skipped(.noDestinations)` increments `photosKeptOnThisPhone` (`noteRoutedPhotoSkip`, audit `mesh.routedShare.skipped`), reset with the session counter, which the app says once as a count (`RoutedShareRefusalCopy.photoKeptNotice(count:)`, the camera's info sheet + film-counter glyph), never as an alert per shot. |
...
| `handlePeerDiscovered(_:)` | Auto-invites peers during proximity join or into an open mesh when capacity/overflow rules allow. Door 1 of the hold: `isAdmittingNewPeers` is read as its FIRST line, above the `isProximityJoin` branch, and again in the detached re-invite retry `handlePeerDisconnected` schedules (finding F-6). |
...
| `disconnectSlot(_:)` | Sends goodbye, cancels coordinator, removes slot/trust policy, and reranks. Reused by `disconnectUncommittedSlotsForHold()`, so a slot the hold drops dies exactly as any other eviction does. |
| `disconnectUncommittedSlotsForHold()` | **P8 item 3 (review finding F-2)**: the hold's sweep. Disconnects every slot with no fingerprint — this codebase's one spelling of "not committed" — through `disconnectSlot(_:)`, so `holdCommittedLinks()` leaves exactly the links it is named for. Runs AFTER `isAdmittingNewPeers` is lowered, so the `armSessionGiveUpClock(now:)` each disconnect ends with is refused instead of lighting the fuse the hold is putting out. Bounded by `slots` (R2), which the loop walks as a value copy. |
| `onSlotConnected(at:identity:)` | Handles every slot commit: the session-roster capture, the shop formation hook, the **founding at the FIRST commit** when this is a proximity-join session with no mesh (P6 item 2), else the descriptor send — then the vouch list and the shared per-commit tail. The roster name comes from `rosterDisplayName(for:)` (2026-09-22): a name-withheld commit (Option 1b) never overwrites a name the session already knows, so a returning member re-seated over a re-dial keeps its first meeting's name even if the link drops before the disclosure repeats; a disclosed name always replaces. |
| `noteCommitIntoMesh(peer:)` | The tail every commit into an existing mesh shares: `applySessionEvent(.peerCommitted)` plus the beacon and the first rotation. Split out at P6 item 2 so the founding path runs the same one — before it, the promotion returned early and a promoted mesh sat at `.idle` with no ceiling. |
| `foundsPairwiseMesh(local:peer:)` | `nonisolated static`, pure: which half of a pair keeps its mesh when both founded (`local < peer` over handshake-verified fingerprints). An **order, not a gate** — it is never consulted at the founding (an asymmetric manual commit would then leave nobody founding) but only at `yieldsNewbornMesh`. Not a security property: the fingerprint is a grindable 64-bit prefix. |
| `promoteToMesh()` | Mints this device's mesh descriptor at the FIRST proximity commit and **founds** it through `foundMesh(_:now:)`; returns `false` when the context could not be sealed, in which case nothing is announced (P6 item 2). Idempotent by refusal: a second entry for one session is a no-op audited `mesh.promotion.refusedExistingMesh`, because `prepareMembershipLedger` would otherwise replace the session's verifier for a different meshID. |
| `announcePromotedMesh()` | Back-fills the session roster for any slot that committed before the founding and sends the descriptor + vouch list to every committed slot. Never called for a founding that failed, and deliberately does **not** call `startSearching()` (re-minting the Bonjour name mid-session drops the link that just committed). |
| `foundMesh(_:now:)` | The founding both founder doors run: `prepareMembershipLedger` → `seedFounderAdmission` (which also mints this device's key advertisement) → `resetSessionStateMachine` → `startSessionCeiling` → `applySessionEvent(.founded)`, unwinding through `unwindNewbornMesh()` at either failure. |
| `unwindNewbornMesh()` | The COMPLETE unwind of a founding given up — `abandonUnpersistedSession()` plus `clearGroupKeyState()` and `clearKeyAdvertisementState()`. The group key is load-bearing: a yielder past one rotation would be refused `droppedStaleEpoch` by the grant it then needs and could never join. It carries `lastRotationBlockReason` ACROSS the unwind (P6 item 2 pass B): `clearGroupKeyState()` nils it, and on the founding-failure path that string is the one surface saying why the founding was abandoned — the diagnosis has to outlive the state it diagnoses. **Named residual:** the unwind nils `sessionCeiling` and nothing re-arms it, so a yielder ends `.idle` with no ceiling, exactly as every proximity joiner has since P3 — latent only while `enforceSessionCeiling` has no shipping caller (P7's poller). |
...
| `handleMeshDescriptor(_:from:)` | Merges, **yields to**, or accepts a descriptor; re-asserts this user's own closed mode (`reassertLocallyClosedMode()`, every merge and not only the yield); **re-publishes the discovery TXT** when the mode or the meshID moved, because a mode that moved by MERGE otherwise kept advertising `meshID` / `meshName` / `memberCount` for the rest of the session; then sends an admission request if the local member is absent. The re-assert runs BEFORE the republish (fix review P3-5) — it republishes anyway, and publishing first briefly advertised the adopted open mesh on the radio of a user who had closed the session. |
| `yieldsNewbornMesh(_:to:from:)` | P6 item 2's repair for the double mint founding at one commit makes reachable: adopt the incoming mesh iff a different meshID, the local mesh is one this device founded and is ALONE in (one member, itself, derived roster ≤ 1 — a fresh joiner's bootstrap roster is also 1, so the descriptor is what separates them), the routed index provably holds nothing minted in the newborn mesh (P8 item 0 (d): other meshes' custody never counts — it used to, and refused every yield for six hours after any finished session), the sender is this device's only committed peer, and either the incoming mesh has more than one member or `foundsPairwiseMesh` says so. Audits `mesh.descriptor.yieldedNewbornMesh`. |
| `holdsNoRoutedItem(mintedIn:)` | Whether the routed store provably holds nothing minted in the given mesh — fail-closed on every non-`loaded` state but `.absent`; a parked (manifest-less) record is a peer's by construction and never counts. |
| `mergeMeshDescriptor(_:incoming:)` | Merges descriptor name/mode by timestamp and appends non-removed new members. A foreign meshID is dropped by name (`mesh.descriptor.droppedForeignMesh`, R7 — newly reachable and formerly silent); `handleMeshDescriptor` then hands the refusal to `reannounceToNewbornPeerIfNeeded`. |
| `reannounceToNewbornPeerIfNeeded(local:incoming:from:)` | P8 item 0, device finding (a): the asymmetric-commit founding race repaired from the side that survives it. Without UWB the two commits are two taps apart; the first tapper's descriptor dies at the second's uncommitted door and the second founds its own mesh, so when the first tapper holds the lower fingerprint it refuses that mesh and nobody ever sends it anything to yield to. On the foreign refusal, when the refused descriptor is a newborn one-member mesh whose member is the sender and this device's mesh would win `yieldsNewbornMesh` from the sender's side (more than one member, or the pairwise order), re-sends this device's descriptor to that slot ONCE per `(sender, refused mesh id)` — `reannouncedNewbornMeshKeys`, cleared with the ledger, capped at `maxNewbornReannouncements`; the peer then yields through the existing path. Audits `mesh.descriptor.reannouncedToNewbornPeer` / `.reannounceSpent`. |
...
| `handleAdmissionRequest(_:)` | Adds a valid unknown requester to pending admission requests, then auto-grants it when `autoGrantsFoundingAdmission(_:)` says so. |
| `autoGrantsFoundingAdmission(_:)` | The founding pair's ONE admission needs no tap (P6 item 2, a **policy act**): proximity-join, open session, an open mesh whose descriptor holds exactly one member (a **latch** — `allowAdmission` appends synchronously, so a roster-only gate would pass two simultaneous requests), a derived roster of one, no rejoin bar, not a removed fingerprint, and a requester holding a COMMITTED slot whose handshake-verified signing key is the one in the request. A closed session, a non-join session and every third-and-later device keep the prompt. |
...
| `persistSessionContext(addingEpochHead:terminating:)` | **The single save seam** for `MeshSessionContext` (one narrow exception since owner-calls item 3: `acknowledgeSessionEndingPresented()` flips only `endingPresented`, on a `.loaded` context of the restored mesh, endings only; a written termination clears the flag) — item 6 extended the cadence here rather than adding a second writer over a five-state load. Durable before acknowledged: false blocks the caller, always. |
...
| `beginBackgroundContinuation()` / `endBackgroundContinuation()` | **P8 item 5: the two public session-state raises, and the first shipping raiser `MeshSessionEvent.backgrounded` / `.foregrounded` ever had.** Each is one line — `applySessionEvent(.backgrounded)` / `applySessionEvent(.foregrounded)` — and exists only so the app can reach an event whose enum is `internal`: `applySessionEvent(_:)` stays internal, and `MeshContinuationRaiseWallTests` pins the raise at one each inside these two brace-matched bodies, the app's calls at one each inside `MeshContinuationDriver.swift`, and `applySessionEvent(` at **zero** anywhere under `App/`. Their one caller is `MeshContinuationDriver` (`taskDidStart()` on the entry into its `running` claim, `taskDidEnd(_:)` on the edges item 4's table completes a delivered task on), which calls them only while it holds a `BGContinuedProcessingTask` — a scene that merely went dark raises nothing, because a mesh with no continued task is suspended along with the process. **They are not radio verbs** and are deliberately absent from `ProximityRunSeamsTests.everyRadioVerbLivesInTheSeamsFile`'s needle list: they start, stop and pause nothing. What `begin` produces is plan §24.1/§25.1's named disagreement — the state leaves `activeForeground`, which CLOSES `mayCommitRoutedHeartLedgerJudgement`, so a continued mesh custodies routed ciphertext and judges no heart — while the routed access gate's own foreground leg is pushed separately by `ProximityRunPolicy` from `ScenePhase`; neither door reads the other's fact, which `neitherLegIsWrittenInTermsOfTheOther` pins in source and `MeshContinuationDisagreementTests` reads as four corners. `end` reopens that third leg and nothing else: the deferred hearts are judged by the re-entry pass the RISING gate leg runs, not by the raise. Offered from a state with no background edge, both are refused by name and audited (`mesh.sessionState.rejected`), never trapped. **No mesh-id widening was needed** — `currentMesh` and `MeshDescriptor.meshID` are already public, so item 6's `MBO.Fernlet.mesh-continuation.<meshID>` identifier reads `currentMesh?.meshID`, whose nil case ("no mesh, no task") is the honest one; the internal `meshID` accessor, which substitutes the all-zero UUID, stays internal and is `MeshIntroductionAuthority`'s. **The pair is also the whole of `MeshContinuationRaising`** (the fix round's F3), the `@MainActor public protocol … : AnyObject` declared beside them and conformed to by the manager in an empty extension: `MeshNetworkManager` is `final`, so the driver's exhaustive sweep could not COUNT the raises against the entries into and exits from its `running` claim without a seam to record through. It widens nothing — both members were already public doors — and it narrows the driver, which now holds the manager as two verbs and cannot spell a third. |
| `sessionContinuationReading` | **P8 item 6: the narrowest public read a background continuation needs, and the only one it got.** Answers `MeshSessionContinuationReading` — monotonic `elapsedSeconds` since this run's ceiling was armed, the run's `budgetSeconds`, and `connectedFriendCount` excluding self — or **nil when no ceiling is armed**, which is the honest answer for a host deciding whether to advance a progress bar at all. `sessionCeiling`, `sessionMonotonicOrigin` and `branchView` all stay internal. Deliberately NOT a field on `MeshSessionPollReport`: that value says what one POLL did — flags, never fingerprints — and exists only on a tick, while this is read at two sites, the SUBMISSION (whose request subtitle carries the friend count) and the tick (whose bar carries the elapsed fraction); a skipped poll would have had to carry three meaningless zeros. The count prefers the branch's external present members — what "friends connected" means once a mesh is larger than a pair — and falls back to this device's committed slots, because `branchView` is built by partition detection and a session's first tick has none yet. Elapsed is derived from `sessionCeilingVerdict(now:monotonicElapsed: nil)`'s remaining seconds rather than the wall clock, which a time change moves backwards — and a progress bar that retreats is the one thing the system ends a task for. |
...
| `evaluatePartition(now:)` / `evaluatePartition(reachable:now:)` | Plan §10.2's partition detection, evaluated **on demand** in the same idiom — no new timer; P7 item 4's `pollSession(now:)` drives it, third in order, from the app's one timer. Re-derives `branchView`, raises `linksLost` / `linksRestored`, and **mints nothing**: the derived roster does not move. |
...
| `restoreSessionContextOncePerLaunch(now:)` / `restoreSessionContextAtLaunch(now:)` / `retrySessionRestoreIfPending(now:)` | The durable half: five load states → seven outcomes, a bounded retry for the two retryable ones, a quarantine for a corrupt file, and **no writer** for any of the three token-less states. **P6 item 7 gave it its shipping callers.** `restoreSessionContextOncePerLaunch(now:)` is the public door the composition root calls — `FernletApp`'s ready-view `.onAppear`, one statement after the launch gate push (`FernletApp.shouldRestoreSessionAtLaunch(alreadyMounted:meshHarnessSeeding:)` is the pure decision, and Lane C's `FERNLET_MESH_MATRIX=1` bypasses it so a re-used Simulator cannot hand run N the context run N−1 left behind). The per-process latch is the MANAGER's (`sessionRestoreAttempts`, monotone and reset nowhere), not the caller's, and it writes its own two frozen audit tokens — `mesh.sessionRestore.launchMount` on the taken path and `mesh.sessionRestore.launchMountRepeated` on a re-fire (counts only, no fingerprint; rowed here by the item's fix review, P3-5, which found them in no index). It answers a `Bool` because `MeshSessionRestoreOutcome` is internal, and it **reconnects nothing**: `currentMesh` stays nil, no radio is armed, and no app surface reads `lastSessionRestoreOutcome`, `offersForegroundResume`, `restoredSessionContext` or `rejoinBar`, so nothing is presented either. What it buys is addressability — the re-proved ledger, its derived roster, the restored key advertisements and the epoch heads — so a custodied routed item can drain the moment a link forms. `retrySessionRestoreIfPending(now:)`'s shipping caller is the routed re-entry's job 1 (`protectedDataRose` ∧ `sessionState == .idle`), which is why a launch before first unlock must still attempt the restore: the attempt is what makes an outcome pending. |
...
| `recordVerifiedAdmissionDurably(admittedBy:)` / `joinDurably()` / `reassertCommitIntoAdoptedMesh(admittedBy:)` | **The join-ack gate**: no epoch adopted, no key unwrapped, no beacon started and no "joined" shown until the context is on the disk. **Since P6 item 6's fix review** the durable arm also re-asserts the commit this device had ALREADY made before it held the mesh it is joining (P1-1): `noteCommitIntoMesh(peer:)` is guarded on `currentMesh != nil`, so the yielding half of a pairwise founding — and any joiner whose dwell commits before the descriptor arrives — had its `.peerCommitted` dropped on the floor and then sat at `joining`, whose ONLY edge out is that event. A steady two-device session raises no further commit, so the heart stage (`sessionState == .activeForeground`, the one routed predicate that reads the session) was false for the whole session and every heart to that device expired custodied while its sender saw "Sent". **Exactly one raise, from `joining` only** — a second would be a self-edge `openBlipMergeIfReconnected` reads as a reconnect and answers with a merge window. **P6 item 9 (the fix review's P3-a/P3-b)** made two corrections: the raise names the GRANT'S OWN SENDER, threaded down from `handleAdmissionGrant`'s authenticated sender, instead of `slots.compactMap(\.fingerprint).first` — an arbitrary committed slot can be a peer the adopted mesh never admitted, and it was inert only because of guards on the far side of the raise. The derived roster is the fallback for a caller with no grant, never the rule: `armJoinerLedger` bootstraps the verifier from this device's OWN admission alone, so at that instant the roster names the joiner and not the admitter (measured — a roster filter there re-opened P1-1's outage). And the re-assert now RETURNS its own durability: the raise persists the context, and answering `true` over a refused seal told the caller a grant was recorded durably when the state change it produced was not. **Item 9's own review (P2-1) made that `false` an UNWIND** (`unwindRefusedReassert(beaconWasRunning:rotationWasScheduled:nextRotationAt:)`): `applySessionEvent` assigns the state before it performs the effect list, so the refused seal left the device at `activeForeground` with the beacon loop running while `handleAdmissionGrant` restored the pre-join ledger underneath it. The machine goes back to **the state the GRANT found** and only the timers THIS raise started are stood down. **P6 item 9's SECOND fix review made three corrections here** (P2-A, P3-1, P3-2). (a) It restored `joining` rather than the pre-state, and on `recordVerifiedAdmissionDurably`'s FIRST arm — the ordinary joiner, `sessionState == .idle` — `joinDurably()` had just created that `joining` on a save that SUCCEEDED, so a refused re-assert left the device durably joining a mesh whose admission record the caller was rolling back: the same half-rolled-back grant one rung lower. The pre-state is now captured at the top of the door and threaded through as `restoringStateTo:`. (b) The two rollbacks were a `guard` chain, so a beacon that was already running skipped the rotation rollback entirely — correct only by a coupling to `noteCommitIntoMesh`'s own early return, with no compile-time link; they are two independent `if`s now, and a cell drives the door with both already up. (c) Door 3's clock needs nothing put back, but not for the stated reason: `armSessionGiveUpClock(now:)` refusing to arm while a slot is committed bounds ARMING, not SURVIVAL. What makes it moot is `evaluateSessionGiveUp(now:)`, which cancels the clock at its next wake once a slot is committed. Test seams behind the unwind's claims: `isBeaconLoopRunningForTesting`, `isRotationTimerScheduledForTesting`, `lastKnownNextRotationAtForTesting` and `scheduleRotationForTesting(nextRotationAt:)` — the last two exist because the rotation half and the SELECTIVITY (a timer that was already up must survive) could not be asserted at all before. |
...
| `resetSessionStateMachine(keepingTerminalState:)` / `abandonUnpersistedSession()` | Clear the run-scoped halves (a terminal state survives until a new session starts), and unwind a founding whose context could not be sealed. `abandonUnpersistedSession()` does **not** delete the sealed context itself — named window: a yielded newborn mesh stays on disk until the next save, and a force-quit inside it relaunches into a one-member mesh nobody is in. |
...
| `checkCoordinatorStates()` | Commits newly connected slots with verified keys and removes stale uncommitted slots. Then runs `reseatReturningMembers()`. A seat is refused, and the link evicted, by three checks in order: `maySeatVerifiedPeer(signingPublicKey:)` (a stranger on a closed mesh), `reseatRefusal(at:identity:)` (a slot the re-seat asked to commit) and — for **every** seat since 2026-09-23 — `seatTransportRefusal(at:identity:)`. |
| `seatTransportRefusal(at:identity:)` / `seatTransportRefusal(committedKey:provenKey:)` | **2026-09-23.** Every seat, whoever asked for the commit — a tap on a slot row, the QR ceremony, a dwell, a heartbeat ratified by local ranging, the re-seat — must commit the key the tunnel's signed channel introduction proved for this link (`MeshTransportSession.verifiedSigningPublicKey(for:)`); a link with no proven key is refused too, because a seat ADOPTS an identity (on the QUIC radio a nil means the slot's tunnel is gone). Evicted and audited once as `mesh.slot.refusedUnprovenIdentityAtSeat` with `reason` = `transportKeyMismatch` / `noTransportKey`. It closes the seat of an identity the transport proved false: a stranger's replayed introduction, a member's introduction the re-seat had already flagged `refusedMismatchedKey`, and a seated link re-committed as somebody else (the coordinator accepts a second introduction in any state). An honest first meeting is never refused — one `IdentityService` signs a device's channel introduction and every envelope its coordinator sends. The static form is the rule alone, for a table cell. |
| `reseatReturningMembers()` / `isReturningMember(signingPublicKey:)` / `reseatsReturningMembers(in:)` / `gatedPeer(of:)` | **2026-09-22.** Commits a slot waiting at a proximity gate whose coordinator-verified signing key is on the **signed** derived roster of the live mesh (states `joining` / `activeForeground` / `continuingInBackground` / `partitioned` only) **and** equals `MeshTransportSession.verifiedSigningPublicKey(for:)` — the key the QUIC channel introduction proved for that link, which the coordinator's replayable identity envelope does not. Through `commitManualProximity(slotID:)`, once per slot (`PeerSlot.returningMemberReseat`: `open` → `commitRequested(signingPublicKey:)`, the judged key), audited `mesh.slot.returningMemberCommitted`. The decision is only an ASK — the coordinator commits whatever identity it holds when the ask lands, and it accepts a second identity introduction in any state — so `checkCoordinatorStates()` re-judges at the seat through `reseatRefusal(at:identity:)`: a slot the re-seat asked for is seated only as the judged key, still the transport-proven key, still a returning member, and is otherwise evicted, audited `mesh.slot.returningMemberRefusedAtSeat` with `reason` = `identityChanged` / `transportKeyChanged` / `notAReturningMember` (most refusals are honest — a lapsed or leaving session, a rolled-back admission — and the reason tells them from a swap) (the re-review's F1: a member replaying another member's introduction on its own link; also refuses a seat whose admission rolled back unsealed, F2). A mismatched key is refused for the life of the slot (`refusedMismatchedKey`, audited once as `mesh.slot.returningMemberRefusedMismatchedKey`) — the re-seat's refusal only: a person's tap or a dwell that commits it anyway is refused one step later by `seatTransportRefusal(at:identity:)` (2026-09-23); a link with no proven key yet stays `open`. Never this device's own key, never a member this device voted out (`removedMemberFingerprints`). Judged from `checkCoordinatorStates()` and from the `didSet` of `membershipVerifier` and `sessionState`, so a member admitted by somebody else while its link waited is re-seated the moment the admission lands. A re-dialed member used to wait at the gate forever — the Connect/Force rows are hidden while a session is up — with every frame it sent dropped as uncommitted. A stranger's gate is untouched. |
| `injectUITestStateIfNeeded()` | Seeds deterministic mesh/admission state from UI test environment variables. |
| `commitSlotForTesting(at:peer:displayName:capabilities:)` / `promoteToMeshForTesting()` | **P6 item 2's two test entries to private production doors, neither a door itself.** The first drives the REAL dwell commit: `maySeatVerifiedPeer(signingPublicKey:)` (evicting on refusal, same audit token), the seat's transport rule `seatTransportRefusal(committedKey:provenKey:)` (2026-09-23; a radio that proved nothing is read as the seam's own handshake — the in-memory rigs' — while a radio that proved a DIFFERENT key is refused as the seat refuses it), then the same four handshake-verified fields `checkCoordinatorStates` writes, then `onSlotConnected(at:identity:)` — whose only production caller is driven by a live `ProximityCoordinator` state a unit test cannot reach, so before it the commit path had NO test entry and the tier-1 rigs faked commits with `applySessionEvent(.peerCommitted)`. The fingerprint is derived from the signing key, so a cell cannot commit a slot whose key and fingerprint disagree. The second enters `promoteToMesh()` directly, which is the only way to fail its idempotence guard: its one shipping caller supplies `currentMesh == nil`. |
...
| `MeshKeyAdvertisementFold.folding(_:into:verifiedBy:)` | The one door. Cheap idempotence pre-filter (identical to a row already verified — the WHOLE value, mesh id and signature bytes included, so a near-miss is still verified and still charged), then **verification**, then the three-way decision. A refusal never aborts the batch: a relayed set legitimately carries rows this device refuses. |
| `MeshKeyAdvertisementFold.restoring(_:verifiedBy:)` | The restore door (P6 item 1 pass B): re-proves every row read back from the sealed context against the ledger adoption just proved, keeps the survivors with their conflict marks, and names every drop `mesh.keyAgreement.rejected`. `MeshLedgerAdoption.adopt` can NARROW the admission set, and a departure or removal in the same blob narrows the derived roster — so a persisted row can be one this device could no longer prove. The file seal is not a substitute for proving it. |
| `MeshKeyAgreementPayload` | The wire frame (`fernlet.mesh.key-agreement.v1`): mesh + a batch clamped to the set's capacity on BOTH the memberwise init and decode + an audit-only `senderFingerprint`. A batch because relaying the whole set is the star fix; **not signed** because every element is signed by its own subject and re-verified against the receiver's own admissions, and unlike the membership digest this frame spends no budget of the receiver's. |
| The frozen audit vocabulary | Pass A: `mesh.keyAgreement.folded` / `…conflicted` / `…setFull` / `…rejected` (an honest replay audits nothing). Pass B adds `mesh.keyAgreement.notDurable` (a fold the store refused, rolled back out of memory — with its VERSION, third review P3 5, so nobody is re-told a set they hold), `mesh.keyAgreement.senderBudgetSpent` (the per-sender frame bound, or the map's own roster-cap growth) and `mesh.sessionRestore.keyAdvertisementsRestored` (counts only: kept and dropped). **The park's six** (second fix review; rowed here by the third review's P3 10, which found two of them in no index and no DocC page): `mesh.keyAgreement.parked` (a refused row took a slot in its authenticated sender's share), `…parkedReoffered` (a widening drained the park — the count, plus `held`, the mesh id this device holds, P9 item 7: the one key a per-cell count can be scoped on over a process-global capture registry), `…parkCollision` (two different rows for one member inside one share, with `kept: newcomer` or `kept: held` as the only thing an operator could not infer), `…parkFull` (the share or the sender bound is spent), `…parkDropped` (a row that failed `failedRosterMovesPerRow` verified roster MOVES is evicted rather than left immortal) and `…parkRolledBack` (a re-offer whose seal the store refused put the park back with the set). English forever, and **counts and the held mesh id only — never a fingerprint**, which is why the member each decision is about rides the outcome value and not the log line. **P9 item 7's fix review gave `held` to the whole family**, not just `…parkedReoffered`: item 6 had put twenty unscoped `== N` counts over these tokens onto the gated CI line beside suites that park rows demonstrably, and one of them had already read 4 for 3 under full-suite load. One door does it — `heldMeshAuditContext(_:)` — and the two lines whose caller holds the verifier (`…parkedReoffered`, `…parkRolledBack`, and `…parkDropped` through its one caller) take the mesh id unconditionally from it rather than through a `currentMesh?` read that could only ever have described an unreachable nil. |
| `MeshKeyAdvertisementReceiveBounds` | `framesPerSenderPerSession` = `capacity × transitionsPerMember` (16 × 3 = 48) — derived, not chosen: an honest sender re-states its set only when the set changed, and one member's row puts a set through exactly **three** transitions in the life of a mesh (folded, then possibly marked conflicted, then possibly released when the derived roster stops naming it — and a departure or removal is permanent, so the release happens at most once). The second fix review recomputed it: the original derivation counted two, so the ceiling (32) sat *below* the reachable version count (48) and an honest sender's late folds could be refused `senderBudgetSpent`. The membership family has no other receive-side limit, and this is its first frame whose per-frame cost can be sixteen Ed25519 verifications. |
| `MeshKeyAdvertisementSendBounds` | `selfMintAttemptsPerSession` = 3 — the bound on the SENDER's repair of its own row. The arm is the normal path and spends none of it; the repair exists because the arm's one moment can fail (a save the store refused, a crash between the arm and the first save) and every other durable membership fact in this module has a repair path. Bounded because the unbounded version was measured, not theorised: a signature plus a failed sealed write on every link-open, for the life of a session whose store cannot be written — it tripled a full test run. Named when spent (`mesh.keyAgreement.selfMintUnavailable`). |
| `MeshKeyAdvertisementPark` / `MeshParkedKeyAdvertisement` / `MeshParkedKeyAdvertisementOffer` / `MeshKeyAdvertisementParkOutcome` / `MeshKeyAdvertisementParkBounds` | The side container for rows refused **`signerNotAdmitted`**, per AUTHENTICATED SENDER (P6 item 1, second fix review). The first shape was one flat map keyed by member, first-arrival wins, capped at the set's 16 — squattable by a single misbehaving *member*, because `signerNotAdmitted` is answered BEFORE the signature check and one frame carries sixteen rows: one frame disabled the park for the session, and junk sitting in member M's slot meant M's genuine relayed row was silently not parked. Three named bounds: `rowsPerSender` (= the set's capacity — a whole frame's worth, which is exactly what the joiner-bootstrap case needs, since it can prove none of what its admitter relays), `senders` (= the roster cap, R3) and `failedRosterMovesPerRow` (3 — not one, because a widening arrives in stages; it is what keeps a bigger container cheap, since a row costs at most three verifications in its whole lifetime, and it counts verified roster MOVES rather than widenings, so three narrowings evict a genuine early row). Within a share **first parked wins** — arrival order, never the set's `precedes` rule, which orders on an unverified row's attacker-chosen `advertisedAt` — and a row that has already failed a roster move yields its slot to a newcomer, audited either way (`mesh.keyAgreement.parked`, `…parkedReoffered`, `…parkCollision`, `…parkFull`, `…parkDropped`, `…parkRolledBack` — the six rowed in the frozen-vocabulary row above). Nothing in here is verified and nothing in here can mark anything. Memory-only, no wipe row owed. |
| `MeshKeyAgreementAdvertisementSet.clearingConflicts(outside:)` | The bounded relief for the conflict mark's blast radius (P6 item 1 fix): drops marks for members the **derived roster** no longer names, keeping every row. A conflicted member refuses the WHOLE mint, mesh-wide and durable across restarts, so the mesh's own answer to a misbehaving member — a departure or a removal vote — has to end the outage rather than leave a mark that outlives the membership. Takes no fence away: a member the roster does not name is never a mint destination, so the mark it drops could refuse nothing today. |
...
| `MeshContentKey` / `MeshContentSet<Item>` | `MeshContentKey` is `(senderFingerprint, contentID)` — **the identity the union keys on since P6 item 4's fix review (P1-1)**, because a routed item's id is its ORIGIN's own choice, the routed index's key is `(originFingerprint, itemID)`, and the signed manifest publishing that id reaches the whole roster-at-creation in the clear *before* the content does; keying on the id alone let an admitted member mint a same-id item and spend another member's dedup slot at a partitioned third device. The key is LOCAL: no wire field, no golden, no persisted surface, and §10.3's order did not move (it already ranked `senderFingerprint` above `contentID`). `contentIDs` still answers the ids held — the inventory question a merge exchange asks — while `mergeKeys` is the set's own identity. All three kinds are keyed the same way, though only the message set has a shipping call site today. The set: dedup by key (the order-least copy wins), sort, keep the newest `setCapacity` — the same bound every live surface applies. `merging(_:)` is commutative, associative and idempotent, at the cap as well as below it. Deliberately **not** `Codable`. |
| `MeshMergedPhoto` / `MeshMergedMessage` / `MeshMergedHeart` / `mergeKey` | The three projections, each identified by `mergeKey` = `(senderFingerprint, contentID)` — derived on the protocol so no conformer can weaken it. The photo carries `keyEpoch` through untouched (§21.5's P5 handoff) and a **local** SHA-256 digest, not a wire field. The message carries both clocks — the untrusted `claimedSentAt` and the receiver's `firstSeenAt`. The heart carries **no** receipt field: a peer's receipt is not this member's. |
...
| `MeshHeartCommit.commit(_:into:)` / `MeshHeartCommitOutcome` | Drives merged hearts through the **existing** `ProximityHeartLedger` — its id-dedup, its 5-minute per-sender cooldown — rather than a second copy of them. `judgements` is one per distinct gift id: a duplicate that crossed the split is collapsed by the union *before* the ledger sees it, so the cooldown is judged once. **It is a BATCH counter and must never be the exactly-once witness** — `MeshRoutedHeartAck.judgementsForGift` is, and `MeshRoutedAck.swift` forbids `outcome.judgements == 1` by name. P6 item 6's ceremony hands this a ONE-element batch, so the two coincide at every shipping call site: an invariant written against the batch counter is green for an accident of that call site forever, which is why the heart cells count `onHeartJudgedForTesting` firings per gift and carry a fixture-only two-heart batch proving the fields differ. |
...
| Type / Function | What It Does |
| --- | --- |
| `MeshDeliveryTarget.init(contentID:roster:selfFingerprint:)` | Captures the destination set = **full derived roster at creation time − self**. The parameter is a `MeshDerivedRoster`, so a branch view or a reachable set cannot be passed by mistake, and **no method removes a destination**. `init(for:roster:selfFingerprint:)` is the one-call form for a `MeshMergeableContent` item. |
| `MeshDeliveryTarget.addressing(contentID:recipient:roster:selfFingerprint:)` | **P6 item 6's third capture door**: a SINGLE-RECIPIENT destination set, still taken from the derived roster at this instant. The recipient is a **caller argument** — never body-derived, because a destination set read out of an origin's own bytes would let it address anyone it named — and it is refused unless the roster-at-creation holds it and it is not this device. A static factory rather than a second `init`, so it can refuse by NAME: `recipientIsSelf` (a caller bug) and `recipientNotInRoster` (an ordinary race — the friend left between the button's render and the tap, and it reaches the user as copy). Fingerprints compare RAW, as everywhere else on the routed path. Self is checked first, because a roster always contains this device. |
...
| Type / Function | What It Does |
| --- | --- |
| `MeshRoutedItemBodyFormat` | The frozen framing: an 8-byte big-endian header length, and the two coder factories (sorted keys, unescaped slashes, dates as seconds since 1970) that make one header value produce one byte string everywhere. Changing an option is a wire decision and moves the golden. **P6 item 3** adds `maxHeaderJSONByteCount` (64 KiB) and the derived `maxFramedHeaderByteCount` — an **allowance, not a refusal**: nothing in the framing bounds a header, and the point of the number is to make a type's ciphertext cap a formula rather than a literal. A header past it eats the payload's room and the sealer refuses the whole plaintext by name. `theHeaderAllowanceCoversAMaximalHeader` measures it against the widest well-formed header (32 participants at the shared name bound) rather than asserting it. |
...
sent to nobody), the last is **visible** as the frozen `MeshRoutedShareRefusal` token on the manager's observed `routedShareRefusal`, forked into localized copy by the app (P5 review finding 5). Deliberately NOT
...
| `MeshRoutedShareRefusal` | `sealFailed`, `destinationNotAddressable`, `mintFailed`, `storeRefused`, `storeUnavailable`, `keyMismatch`. Frozen English `rawValue`s — audit vocabulary, never user copy; the app forks each into one `LocalizedStringKey` sentence (`RoutedShareRefusalCopy`, exhaustive by construction). **Amended by P6 item 1:** `destinationNotAddressable` now means a destination NO verified source has stated a key for — the resumption case became a delivery, and the star and over-cap cases became a successful mint whose delivery waits for a link or a departure hand-off. `keyMismatch` is the new fail-closed refusal: two verified sources that disagree, or a member the advertisement set has marked conflicted. |
...
| `sweepingExpired(now:)` / `sweepingOrphanChunkFiles()` / `dropping(item:reason:)` | On demand, never on a timer (and not by P7 item 4's `pollSession(now:)` either — expiry sweeps ride the drain path). The orphan sweep's loop is bounded by **2 ×** `maxHeldChunkFiles` and reports `sweptToCeiling`, because a directory that got over the cap is exactly what orphans produce. |
...
| Type / Function | What It Does |
| --- | --- |
| `MeshRoutedTypeEntry` | One row: `token`, `maxItemByteCount`, `destinations`, `relayRetention`, `finalAck`, `expiry`, `canonicalStore` — plus `requiresForegroundDecryptBeforeFinal`, **derived** from `finalAck` so two fields cannot disagree about one policy. Its doc carries the normative rule P6 is held to: once a token is registered its `finalAck` and `destinations` are as frozen as the token (a `finalAck` disagreement refuses nothing and diverges SILENTLY), a type whose semantics change gets a new `…v2` token beside the old row, and only the cap and the canonical store may be edited in place. Its **unit caveat** is the thing to get right when narrowing a row (P6 item 3): `maxItemByteCount` is compared against `MeshRoutedManifest.size`, the complete sealed CIPHERTEXT blob, while a store's byte and pixel bounds are PLAINTEXT bounds enforced at reassembly — so a narrowed row is written as a formula (payload plaintext bound + `MeshRoutedItemBodyFormat.maxFramedHeaderByteCount` + `MeshRoutedItemSealFormat.overheadByteCount`), never a literal. |
| `MeshRoutedDestinationSemantics` | `fullRosterAtCreation` (D7/D12 — photos and text) and `singleRecipient`, **minted by the heart row since P6 item 6**, which flipped that column in place: the one re-declaration the entry doc's freezing rule allows a never-minted row, and it is spent. Two readers, two meanings: `MeshNetworkManager.originateRoutedItem` checks it against the **audience its caller states** (the real fence, and the only one for a full-roster row handed a single recipient), and `MeshRoutedManifest.validated` checks a **shape** — a `.singleRecipient` row must arrive with one destination, which is non-vacuous on a roster of three or more and vacuous on a pair. Still **no receiver-side reader**: the manifest carries its destination set on the wire and the verifier binds wraps ≡ destinations from those bytes. |
...
| `MeshRoutedParkedDrop` / `.Reason` | The ONE-clause terminal-refusal rule: `unknownTypeToken` **and** the frame's sender is the manifest's own claimed origin. Every other `MeshRoutedManifestRejection` keeps the bytes, each for a stated reason — including **P6 item 3's** `sizeExceedsTypeCap`, whose arm states the asymmetry: an unknown token is terminal under any build that does not register it, while a cap is a NUMBER one build chose and a later build may loosen, so the same parked set is deliverable tomorrow; a cap-driven drop would also be a remote delete lever (one over-cap manifest and the parked bytes are gone). Expiry collects them if the cap never moves. Origin-bound because that rejection is checked before the signature and the origin is the only party the chunk door lets park a set; a clause on `notADestinationOrHandoff` was designed and removed as a remote delete lever. |
...
| `routedTypeCapRejection(for:)` | **P6 item 3 / D-11.4**, the manifest door's per-type size check. Compares the origin-signed `manifest.size` — the complete sealed CIPHERTEXT blob — against the registry row's `maxItemByteCount`, answering `.sizeExceedsTypeCap`, or `.unknownTypeToken` when `entry(for:)` is nil (the same answer the verifier's accepted-token set gives, so the door carries no default; unreachable in one build because the set is a projection of these rows). Runs AFTER the verifier — a statement about what the origin signed, not about what a relay changed — and BEFORE the destination/hand-off gate, so a custodian's copy and a destination's copy of an over-cap item are refused identically. One dictionary lookup and one comparison; no I/O, no crypto. **No parked-set drop hangs off it:** `dropParkedSetIfTerminal` keeps its single verifier-branch call site, and a cap refusal keeps the bytes. |
...
| Type / Function | What It Does |
| --- | --- |
| `originateRoutedItem(body:typeToken:itemID:audience:now:)` | The sender door. Roster → `originationTarget(itemID:audience:roster:typeToken:)` → verified recipient keys → mint → stage → push. **`audience:` since P6 item 6** (`RoutedOriginationAudience.fullRoster` / `.recipient(_:)`): the column's real reader, stated by the caller and checked against the type's registered `destinations`. A mismatch is `.refused(.mintFailed)` plus `mesh.routedShare.destinationSemanticsMismatch`, while a heart addressed to THIS device is the same `.refused(.mintFailed)` under its own frozen token `mesh.routedShare.recipientIsSelf` (P6 item 6 fix review, P3-4: the refusal was always right, the token named a different fact), and deliberately **no new `MeshRoutedShareRefusal` case** — it is unreachable from the two shipping callers, so a new case would owe two copy forks, a catalog row and an `allCases` pin move for a condition no user can reach. `recipientNotInRoster` answers `.skipped(.noDestinations)`, which is the right vocabulary rather than a fudge; whether the skip SPEAKS is the caller's decision (silent for a photo, spoken for text and for a heart). Three answers, and only the third reaches the user: a **skip** when there is no destination set at all — a derived roster of exactly ONE: a solo host, and the founder's window between the founding and the first admission grant (since P6 item 2 the proximity-join pair founds WITH a ledger at its first commit, so the pairwise phase is no longer an instance of it) — a **staged** key, or a named **refusal** for a mint that was attempted and failed. It runs **no verifier** on its own mint (nothing is being received) and never calls `finishLocalRungs` — an origin stages, offers, and acknowledges nothing to itself. |
| `routedDestinationKeys(for:)` / `handshakeVerifiedKeyAgreementKeys()` / `advertisedKeyAgreementKey(for:)` | The wrap keys, from **verified sources only** (D-13.1), now of two KINDS and answering a three-case `RoutedDestinationKeys` rather than an optional (a nil cannot tell "nobody has stated a key" from "two verified sources disagree"). **Tier A, present-tense:** a live slot's `verifiedKeyAgreementPublicKey` and the session-roster entry written from that same verified value — and two tier-A sources that disagree refuse rather than letting the later writer win. **Tier B, durable (P6 item 1):** the verified `fernlet.mesh.key-agreement.v1` advertisement, read only for a fingerprint the CURRENT ledger still names, which is the fence over a grow-only set against a roster that shrinks. Precedence is tier A; **both present and unequal, or a conflicted member, is `keyMismatch`** — fail closed, because `ensureProvisioned()` mints the pair together so one fingerprint cannot legitimately hold two keys. Still refused by name: `MeshMember.keyAgreementPublicKey` from descriptor gossip and the peer-relayed claimed key a grant wrap uses. Still refuses the WHOLE mint rather than a subset — that arrives with item 6's `.singleRecipient` flip. |
| `sendKeyAdvertisements(to:)` / `writeKeyAdvertisementFrame(_:to:at:)` | Relays this device's whole verified set to NAMED committed peers (non-optional by security, not style: `broadcastMembershipFrame`'s nil path writes every slot, uncommitted included, and this frame is unsealed and enumerates the roster). Re-asserts the self row through `repairOwnKeyAdvertisementIfMissing()` so "once and never again" has a repair path — **below** the roster filter, so a peer's link flap cannot burn all three attempts with nothing owed, and gated on `!members.isEmpty` rather than on somebody being OWED the current version: the owed form starved the repair in the one state it exists for (every member latched, self row still missing after an arm-time seal the store refused), forfeiting attempts 2 and 3 for the session (second fix review). `writeKeyAdvertisementFrame` claims the latch for every owed peer **before** the write and releases whatever `writeMembershipFrame` did not reach: latching only after the write left nothing claimed while it was in flight, so a second MainActor door recomputed the same `owed` and wrote the frame twice; latching before it and keeping it turned one lost frame into a session-long `destinationNotAddressable` on a stable pair, because nothing re-arms the latch except the local set changing and no door fires on a fold. Bounded **once per (peer, set version)** for a peer the wire reached: told once, told again only when the set actually grew or gained a conflict mark — not `reGossipedToFingerprints`' spent-forever rule, which would keep a peer from passing on a key it learned in between. A peer whose writes keep failing stays owed and is re-sent at every reconnect, which `askOneReconnectedPeer` does not bound: the cost is bounded per attempt (one encode, one signature) and is zero when no slot carries the fingerprint. Six sites: the three ask doors, the two non-ask membership doors, and the admitter's `grantAdmission(to:meshID:)` — the one link-open no other door covers, since an admission is deliberately not a reconnect. |
| `receiveKeyAdvertisements(_:from:)` / `acceptsKeyAdvertisementFrame(from:verifiedBy:)` / `chargeKeyAdvertisementFrame(to:)` / `commitKeyAdvertisementFold(_:)` / `adoptKeyAdvertisements(_:)` / `rollBackKeyAdvertisements(to:version:)` / `armOwnKeyAdvertisement(now:persisting:)` / `repairOwnKeyAdvertisementIfMissing()` / `restoreKeyAdvertisements(from:verifiedBy:)` / `clearKeyAdvertisementState()` | The receive half and the lifecycle. Refuses a foreign mesh and a malformed frame by name, charges the frame to its AUTHENTICATED sender (the committed slot's fingerprint, never the frame's own field) against `MeshKeyAdvertisementReceiveBounds`, folds, audits per outcome and saves — **durable before it counts**: a set the store refused is rolled back out of memory, with its conflict marks, because they are one value. Deliberately **outside** `MeshRoutedRefusalBudget` and the replay window: this is digest-family traffic (D-5.12/D-6.10), a statement about state rather than a delivery, and it never enters the routed dispatch at all. The self row is minted after the founder's `seedFounderAdmission(meshID:)` and inside `armJoinerLedger(_:now:)` (the joiner passes `persisting: false` so the join's own durable acknowledgement carries it, and its rollback unwinds both); `repairOwnKeyAdvertisementIfMissing()` is its bounded repair and `adoptKeyAdvertisements(_:)` is the one seam the set moves FORWARD through, so the set and the send version cannot drift; `rollBackKeyAdvertisements(to:version:)` is the only other assignment and it restores BOTH (third review P3 5 — a refused seal used to bump the version twice and end on the set every peer already held, so every member was re-sent an unchanged frame charged against its own 48). `acceptsKeyAdvertisementFrame(from:verifiedBy:)` is the sender gate (P6 item 1 fix): the derived roster, plus the one exception `bufferedForAdoption(_:from:)` already makes — a joiner's own admitter while the ledger is still the bootstrap that admitter rooted — so the per-sender map's roster cap is true by construction instead of by coincidence. |
| `parkKeyAdvertisements(_:outcomes:from:)` / `foldParkedKeyAdvertisements()` / `reparkRefusedKeyAdvertisements(_:outcomes:)` / `clearConflictMarksForNonMembers()` | The receiver-side half the grant door needed (P6 item 1 fix, found by item 2's audit; re-shaped by the second fix review). A row refused **`signerNotAdmitted`** is parked in its AUTHENTICATED SENDER's own share of `MeshKeyAdvertisementPark` — see that type for the three bounds and why the share is per sender — and re-offered through the one fold door at each widening (`attemptLedgerAdoption`, the merge door, the live record insert). Every re-offer is re-decided by that door: folded when it has become provable, dropped and named when it now fails for another reason, re-parked one failure heavier while the signer is still invisible (a widening arrives in stages — a third device adopts the moment the chain to its OWN admission proves, which can be a two-member ledger), and **dropped by name** once it has failed `failedRosterMovesPerRow` verified roster MOVES — not widenings, which is the honest unit as of the third review: the re-offer fires on any move, so three narrowings evict a genuine early row, audited (`mesh.keyAgreement.parkDropped`) and re-sent by the sender at its next set change — so a row no admission will ever name is not immortal. A re-offer whose seal the store refuses rolls the park back WITH the set (`mesh.keyAgreement.parkRolledBack`): the set's own rollback lives in `commitKeyAdvertisementFold`, and without the park's half a row that had become provable but whose save failed was in neither container, with nothing to re-send it — the sender's latch is spent at that (peer, version). A parked row is raw bytes that have verified nothing and can mark nothing. `signerNotAMember` deliberately never parks — that is a ledger that names the signer and has subtracted it. `clearConflictMarksForNonMembers()` rides the same seams as the relief for the conflict mark's blast radius. |
| `writeMembershipFrame(_:_:to:)` | `broadcastMembershipFrame(_:_:to:)`'s reporting twin, returning the fingerprints the wire write actually reached. Separate rather than a `@discardableResult` on the wrapper because the house rule is that a send's success value is used or not returned: the thirty-odd senders that cannot act on a failure keep calling the wrapper, and the one that must — `sendKeyAdvertisements(to:)`, whose per-peer latch would otherwise mark a peer told about a frame written to nobody — calls this. |
...
| `shareRoutedPhoto(itemID:addedAt:imageData:session:)` / `noteRoutedShareRefusal(_:error:)` / `routedShareRefusal` / `clearRoutedShareRefusal()` | The photo caller and the one user-visible half of a refusal: the frozen `MeshRoutedShareRefusal` token published on the observed `routedShareRefusal` (the app forks it into localized copy — `RoutedShareRefusalCopy` — and clears it when the alert is dismissed; a new mesh clears it with the hold) plus `mesh.routedShare.refused` carrying the same token. No sentence is composed in the package any more (P5 review finding 5). No third `MeshRoutedDeliveryHoldCause` — that observable states what this device HOLDS, not what it failed to send. |
| `refuseRoutedFrameBeforeStore(event:_:in:charge:)` / `routedRefusalIsHeldForCapacity(_:from:)` / `routedRefusalBudget` / `routedRefusalBudgetForTesting` | **P5 review finding 3, closing D-12.14.** The one door for a routed content frame refused BEFORE any store verb — the named audit line the door always wrote, plus one charge to `MeshRoutedRefusalBudget` against the **envelope sender**; `charge: false` for the one exemption, a frame for an item this device already capacity-refused from this sender (`routedRefusedKeys`), which is the receiver's own refusal arriving in pieces. Called from every such exit: the four undecodable decodes in `dispatchRoutedContent`, the manifest verifier rejection and `notADestinationOrHandoff`, the chunk door's `unknownItemNotFromOrigin` / `unregisteredTypeChunk` / verifier rejection, and both receipt doors' verifier rejections. The budget's one transition writes `mesh.routedDrain.refusalBudgetSpent`; after it `dispatchRoutedContent` drops the sender's content before the decode, unlogged. Reset inside `clearRoutedDrainState()` only. The two digest doors stay outside (D-5.12 / D-6.10). `MeshRoutedRefusalBudgetTests` pins the door, its call sites, the gate and the reset. |
| `projectRoutedItemIfPermitted(key:manifest:seenAt:)` / `routedProjectionVerdict(key:manifest:seenAt:)` / `dispatchRoutedPlaintext(_:store:author:manifest:seenAt:)` | The receiver's derivation, and an item's **only** route to plaintext. Order is load-bearing: refuse outright while a delete-all wipe is in progress (`privacyWipeInProgress`, added at item 4's fix review — above the OFFER, so the item is not even stamped, and `refusedForNow` + uncharged, because a wipe is the gate's own answer for every item) → record the item's transcript GENERATION (before every guard, including the decrypt one — an item offered while the gate was closed still belongs to the generation it arrived in) → decrypt predicate → registry row → already-projected → author from the admission ledger, with the block list and the removal set applied there → `manifest.size` against the seal's resident bound → **mutation predicate, hoisted above the open** since P6 item 4 (with two arms it would otherwise appear twice, and refusing here decrypts nothing it would discard) → reassemble → the per-store arm, which opens its own body. `dispatchRoutedPlaintext` switches on `entry.canonicalStore`, a RESOLVED value the type-token wall permits by name, and never on a token spelling. Every arm returns a `MeshRoutedProjectionVerdict`, and `noteRoutedItemProjected(_:)` is written by ONE caller on `leavesTheRetryList` — the mark now means "handed on, or refused for a reason that cannot change". The store read is `routedProjectionBlob(key:manifest:)`, which switches on the five-state outcome rather than guarding one case out of it. `seenAt` is passed in, never read here: the re-entry pass supplies `MeshRoutedItemRef.firstSeenAt` (already stored) and the live door supplies its own injected instant. |
| `routedProjectionAuthor(for:)` | Fail-closed attribution (D-13.21), resolved against **the same set that admitted the item**: `admissions − removals`, departures never consulted, which is `MeshRoutedManifestVerifier`'s own door (D-13.33). `roster.members + roster.barred` IS the admitted set. Reading `members` alone would refuse increment 1's headline case — §11's custody-transfer-on-departure has the origin leave, hand its outstanding items on, and the custodians deliver afterwards, by which time every destination's derived roster already excludes it, so the transferred custody would be ciphertext no wall could open while the origin read `delivered`. A quorum removal still refuses, by name (`mesh.routedProjection.originRemoved`). The routed body carries **no identity claim at all**, so the wall entry's fingerprint is the signed `manifest.originFingerprint` and its signing key is the ledger's key for it; if the ledger is gone — after `leaveMesh()` — the projection refuses, keeps custody, and writes **no** entry with a nil, empty or body-supplied key. The block check is hoisted here, on the signed fingerprint, before any unwrap. **Three answers since P6 item 6, not two** (`RoutedProjectionAuthor`): the projection collapses both refusals to `refusedForGood` and is unchanged, while the heart ceremony needs them apart — it MARKS its final refusals, and a mark on the wrong leg is either a lost heart or a starved allowance. "The ledger cannot resolve this origin **yet**" is retryable; a removal and a local **block** are durable. The block leg is live, not hypothetical: blocking does not remove a member from the derived roster. |
| `openedRoutedPhotoBody(_:manifest:)` / `openedRoutedTextBody(_:manifest:)` / the two `routedCanonicalDispatch(…)` overloads (the heart's opener is `openedRoutedHeartBody`, and its canonical write is the ack ceremony rather than a dispatch arm — see `routedAckEvidence` below) | The manager side of the one plaintext seam, and the canonical writes themselves. The PHOTO arm produces the same effects the legacy `.friendPhoto` handler produced, through the same functions (`sanitizedIncomingPhoto` → `cachePhoto` → the wall, the FIFO cap, the preference pruning, the sealed `PrivateMediaStore` index, and the closeness hook). The TEXT arm (P6 item 4) produces the same effects the legacy `.tempMessage` handler produced, through `SessionMessageStore.receiveIncoming` — dedup, the sanitizer, the display-name moderation, the cap and §10.3's ordering all stay in the store, and the arm adds only what the store cannot know: `transcriptLiveness(for:)` and `allowIncomingRoutedText`. Both arms correct the same two things rather than reproducing them: the attribution's fingerprint comes from the signature and the ledger instead of an unsigned claim or the live transport identity, and the block list is applied before the unwrap. Neither takes a clock. The spelling's call-site count is pinned at **4** (two declarations + two callers) by `everyRoutedPlaintextSeamNamesItsPredicate`, and `dispatchRoutedPlaintext(` at **2** beside it (item 4's fix review, P2-2: the hoisted mutation guard sits one level ABOVE the inner verb, so a second caller of the outer one reached both canonical stores ungated while the inner pin stayed at 4). Both name the mutation predicate in the wall's containment half, so a third, ungated mutation is a build failure. |
| `allowIncomingRoutedPhoto(_:from:)` / `allowIncomingRoutedText(_:from:)` / `noteRoutedItemProjected(_:)` / `routedProjectedItems` / `routedOriginPhotoQuota` / `routedOriginTextQuota` | The incoming per-origin budgets, keyed on the **item's** mesh from the signed manifest and never on the live one (D-13.23): the legacy counters reset whenever `currentMesh` changed, which was sound while the check always ran inside the session that produced the content, and would hand one origin a fresh budget at every later access-gate edge on the routed path. Re-sends of an accepted id are free in both. The TEXT quota (P6 item 4, `maxTextMessagesPerSenderPerSession` 200) is a **policy act**: it replaces the live-arrival token bucket, which was wrong twice over on a store-and-forward path — a drain answer carries up to 16 items, so a burst allowance of 5 flood-dropped 11 legitimate messages, and a fixed per-item first-seen makes the refill interval zero forever. A per-second rate limit is therefore replaced by a per-session TOTAL; the drain's 16-item answer and its pacing are the rate. Bounded at 1024 keys × 200 ids, and `internal` rather than `private` since item 4's fix review so its three legs have a behavioural cell (`MeshRoutedTextDeliveryTests.theIncomingTextQuotaRefusesPastItsCapAndIsFreeForAReSend`) instead of only a constants pin — making the body `return true` used to redden nothing. `routedProjectedItems` is `private(set)` for the same reason: the two "…AndIsMarkedFinal" cells now OBSERVE the mark. The projected set is marked even when a quota refuses (D-13.29) and now also on every other permanent refusal; the projected set and the generation map are cleared by `clearRoutedDrainState()` and the **quotas deliberately are not** (D-13.23a). |
...
| `beginPrivacyWipe()` / `endPrivacyWipe()` / `privacyWipeInProgress` | **P6 item 4 fix review, P2-1.** The app's delete-all funnel drops the live transcript at its top through `clearSessionTranscript()` (so the generation moves) and switches the routed projection off until it finishes. Both halves are needed: the sealed routed store holding the ciphertext beneath the transcript is not destroyed until leg 11, and every suspension point in between was a window in which a rising access edge re-projected the just-wiped messages into a transcript that is still live, in the same mesh. Paired from a `defer`, so no exit leaves the projection off for the process; memory-only, and the wipe's own state rather than a new persisted surface. **A DEPTH counter since P6 item 7** (item 4's fix review, P3-2): two funnels are reachable at once — `DeleteEverythingFlow.runWipe` sets `isDeleting` rather than checking it, and the duress purge hook fires the same funnel with no UI gate — and both are `@MainActor`, so with a `Bool` the inner funnel's `defer` lowered the flag while the outer one still had leg 11's routed-store purge ahead of it. `privacyWipeInProgress` is now derived (`privacyWipeDepth > 0`), begin saturates at `maxPrivacyWipeDepth` (8) with `mesh.privacyWipe.depthExceeded` rather than growing without bound, end is a no-op at zero, and `mesh.privacyWipe.ended` is written only when the gate really falls. **Saturation does not cost the gate** (the item's fix review, P3-7): a begin past the cap is counted in `privacyWipeOverflow` (itself capped, and audited `saturated` past that), and an end drains the overflow BEFORE the depth — so the gate falls on the last paired end rather than one end early. **It does not make the audit lines a pair, and never did** (P6 item 10 SET A, P3-a): `mesh.privacyWipe.began` is written on every begin and `mesh.privacyWipe.ended` only at depth zero, so N nested funnels write N begins and one end — and an end that drains the overflow returns before writing anything. Past `2 × maxPrivacyWipeDepth` the begin is audited `saturated` and dropped, and the gate does fall one end early again; both the cap and the `saturated` arm are pinned by `overlappingWipeFunnelsKeepTheProjectionShutUntilTheLastOneEnds`. |
...
Network migration P7 item 4: the ONE poll seam the app's run policy drives, and the joiner-side ceiling arm it needs to mean anything. No timer here — the app owns it (`ProximitySessionPoller`).

| Type / Function | What It Does |
| --- | --- |
| `MeshNetworkManager.pollSession(now:)` | **Public.** The three on-demand consumers in their one order — `enforceSessionCeiling` → `evaluateIdleLapse` → `evaluatePartition` — for a live session, `.skipped` for a dead one, returning early when the ceiling ended the session. Audits only a poll that moved something. `MeshSessionPollReport.sessionLiveAfter` is the app timer's stop condition. |
...
| `MeshNetworkManager.sessionResumePresentation` | **Public.** The one app read over the four observation-ignored restore surfaces; derived, never stored; sampled on appear. `SessionResumeCopy` (app) forks it into `LocalizedStringKey` cards. An ending whose context records `endingPresented` answers `.nothing` (owner-calls item 3, 2026-09-22). |
| `MeshNetworkManager.acknowledgeSessionEndingPresented()` | **Public**, the resume card's `.onAppear`. Marks the restored context's ENDING as shown by writing `MeshSessionContext.endingPresented` into the same sealed file, so the next cold start is silent about it. Keyed on `lastSessionRestoreOutcome` — the value the card is drawn from — because opening Friends runs `startJoin()`, which nils `restoredSessionContext` before the card appears (the verify's BLOCKER: keyed on that, it never wrote on a real launch) — the fix for "You left the previous session" recurring at every launch (the context is never reaped: the durable rejoin bar is re-derived from it). Endings only; this launch's card stays up; a deferred/refused/corrupt load or a refused seal writes nothing and is audited (`mesh.sessionRestore.endingPresentedNotWritten cause=load|seal`); success audits `mesh.sessionRestore.endingPresented`. |
...
### `NetworkMeshSession.swift` / `MeshLinkTable.swift` / `MeshTransportSelection.swift` — the mesh radio's proofs and claims (2026-09-23)

Not a full index of the mesh radio — its decisions are documented on the ProximityKit landing page's
Transport section. These rows are the functions the seat check, the payload door and the `sid`
hardening read or added. The squat they close: the `sid` in the channel hello is unsigned and public in
every TXT record, so a verified peer could claim an absent member's `sid`, take its browsed key and
lock its re-link out until the claimant's tunnel ended.

| Function | What It Does |
| --- | --- |
| `MeshTransportSession.verifiedSigningPublicKey(for:)` / `NetworkMeshSession.verifiedSigningPublicKey(for:)` | The key the tunnel's signed channel introduction — bound to this connection's TLS exporter, so not replayable — proved for a peer's live link, nil when none. Three readers, and nil means something different to each: every seat (refused), the returning-member re-seat ("cannot tell yet"), the payload door (contradicts nothing). The protocol default is nil. |
| `admitVerifiedInbound(_:pendingKey:)` / `inboundKey(for:pendingKey:)` | Keys a verified inbound tunnel under the browsed advertisement its claimed `sid` names only when `MeshLinkTable.claimResolves(_:to:heldBy:)` finds nothing verified that contradicts the claim, otherwise under its own connection key — what an inbound tunnel whose `sid` matches nothing browsed has always done. The owner's invitation gate is asked about the key the tunnel will actually live under. A member is never refused because another key holds the advertisement its `sid` names. |
| `activate(_:connection:stream:datagrams:verified:)` | Besides marking a tunnel live: a DIAL's verified key is recorded as the proven owner of the advertisement it dialed (`MeshLinkTable.noteProvenOwner`), BEFORE the duplicate collapse — the proof is the introduction over a connection to that advertisement's own endpoint, whether or not this tunnel survives. |
| `reproposeIdleBrowsedPeers(now:)` | The five-second re-dial sweep. "Already connected under another key" is asked of `MeshLinkTable.sweepSkips(_:liveSessionIDs:liveSigningKeys:)`, so a claimant's tunnel carrying a member's `sid` no longer keeps that member from being re-dialed. |
| `MeshLinkTable.noteProvenOwner(_:signingPublicKey:)` / `provenOwner(of:)` / `provenOwnerCount` | Which key a dial of this device's proved answers a browsed advertisement — the one fact about an advertisement that is not a peer's word. Bound to the endpoint, not the `sid`; bounded by the endpoint cache (not recorded for an uncached key; dies with the entry in `forget`, the oldest-first eviction and `removeAll`). |
| `MeshLinkTable.claimResolves(_:to:heldBy:)` | Whether a claimed `sid` may key an inbound tunnel under that advertisement: no when a dial proved a different owner, or a live tunnel verified as a different key holds it. Neither can move an honest peer — a `sid` is a per-launch random UUID. **Residual:** an advertisement no dial has proven and no tunnel holds can still be named first by a claimant; when this device is the preferred dialer toward it, the claimant holds that key until its own tunnel ends — availability, never identity. |
| `MeshLinkTable.sweepSkips(_:liveSessionIDs:liveSigningKeys:)` | Whether the sweep passes over an idle browsed advertisement: with a proven owner, whether that IDENTITY is among the live tunnels; otherwise whether its `sid` is (the old rule); an advertisement with no `sid` is passed over, as always. |
| `rememberBrowsedForTesting(_:instanceName:advertisement:)` / `bookTunnelForTesting(_:role:verified:)` | Tier-1 seams: the endpoint-cache half of `noteBrowsed` minus the Bonjour endpoint a unit test cannot build, and a booked tunnel exactly as `replaceTunnel` + `activate` record one — including, for a verified `.initiator`, the proven owner. |
