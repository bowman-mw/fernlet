...
| `hasCommittedPeer` | Whether any slot holds a committed fingerprint — the **lifecycle** half (P6 item 2). Read by the three session-end hooks, `ConnectView.presentDisconnectReviewIfNeeded()`, `ConnectView.handleCommittedPeerChange(hadPeer:hasPeer:)` (the keep-prompt abandonment and the connection choreography — on `isInSession` that arm's `!was && now` leg is DEAD for a founded pair), `ContentView.stopFriendsDiscovery()`, the 5-minute discovery timeout, and `startFriendsDiscovery()`'s middle arm. On `isInSession` none of them would ever fire again once a pair founds a mesh at its first commit. `isInSession` keeps the surfaces that must survive a blip: the layout swap, the camera chrome, and `handleSessionSurfaceChange(wasInSession:nowInSession:)`. |
...
| `resumeSearchingForPartitionedMesh()` | **`startJoin()` minus every reset** (P6 item 2): re-arms `isProximityJoin` + the radios for a session whose founded mesh outlived its links, keeping the mesh, the ledger, the ceiling, the advertisement set, the photos, the film quota and the state machine. `ContentView.startFriendsDiscovery()`'s middle arm (`isInSession && !hasCommittedPeer`) is its one shipping caller. Without it a founded pair that blipped and then left the tab had no way back in — `startSearching()` is private and the `!isInSession` guard passes over a mesh that outlived its link — and `startJoin()` is not the answer: it would nil the ceiling on a mesh that can never re-found. |
...
| `reassertClosedModeAfterYield(_:)` | Re-applies this device's own **closed** choice to the mesh it adopted in a yield (P6 item 2 pass B). Without it the yielder inherits the winner's mode and a user who closed the session before the dwell has it silently reopened — and an open mesh's TXT publishes meshID/meshName/memberCount. Legitimate because ANY member may close: `setMeshMode(_:)` gates on nothing but holding a mesh and the merge resolves `mode` by last-write-wins with no founder check. Called after the admission request so the request reaches the winner while its one auto-grant still applies. |
