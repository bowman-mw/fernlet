...
| `MeshRoutedDrainAnswerPayload` | The signed frame — answer, `senderFingerprint`, floored `sentAt`, signature — with its own shape check for the four scalars no other door clamps. Signed rather than unsigned because item 7's merge window closes on quiescence, so a forged `true` closes a window that should still be open. `signed(meshID:advertiser:advertisedAt:quiescent:sentAt:identity:)` takes no `selfFingerprint`. |
...
| `MeshRoutedPeerInventory` | Per-peer, in memory, session-scoped: the peer's last verified holdings and its `sentAt`, the `advertisedAt` this device last sent it, and **both** halves of `converged(local:peerReportsQuiescent:)` — so item 7's window rule is a pure read rather than a second main-actor `load()` and comparison. |
...
| Type / Function | What It Does |
| --- | --- |
| `sendRoutedInventory(to:now:)` | The routed twin of `sendInventoryDigest(to:)`, called from **exactly** its three call sites (`beginMergeExchange`, `askOneReconnectedPeer`, `handleAdmissionGrant`'s reply) and nowhere else — the grep-walled form of "no second reconnect path". Records the **minted payload's** floored `sentAt` per peer, never `now`. |
| `routedIndexForAdvertising()` | The fail-closed five-state switch. `.loaded` and `.absent` advertise (the latter over an empty index — `load()` answers it from the file read, before the seal key is consulted, so it cannot be a locked-device artefact); `.deferred`, `.corrupt` and seal-`refused` advertise **nothing**, and `routedDrainPlan` returns nil for them too, so a non-vending store sends no answer frame at all rather than skipping the bulk inside one. |
...
| `clearRoutedDrainState()` | Called at the three session resets `peerInventoryDigests` is cleared at, and nowhere else — **not** `abandonMergeExchange`, where a flapping link would re-spend this device's bytes on every flap. |
