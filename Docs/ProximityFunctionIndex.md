...
| `dispatchMembershipEventPayload(_:plaintext:decoder:slot:)` | Decodes `.meshMemberDeparture` / `.meshMemberRemoval` / `.meshTerminated` / `.meshInventoryDigest` on a COMMITTED slot, inserts through `MeshMembershipRecordVerifier`, commits durably, then rotates — or, when the record names THIS device, applies §8.2's `removed` edge and tears down instead. |
| `insertMembershipRecord(_:plaintext:decoder:)` | The decode-and-insert half, split out so the dispatch stays under the 60-line rule. Returns the removed fingerprint **only** for an ACCEPTED removal — a refused record must not be able to end anybody's session. |
...
| `emitApprovedRemovalRecord(_:)` | Mints, files and now BROADCASTS the removal record when a vote completes (P3 item 3b). A record the store could not seal is rolled back and never announced. The legacy `.meshRemovalSecond` + descriptor path stays: it is the interim exclusion authority until item 7. |
