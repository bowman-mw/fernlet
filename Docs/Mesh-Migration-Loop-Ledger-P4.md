...
**Started:** 2026-09-02 · **Iteration:** 8 done · **Tree at seed:** main = `262f3da` (pushed; = `3722511` + the launcher commit)
...
## Next item
8 — delivery-target semantics for P5 (§10.1, §5c; prereq 1): a named type answering "who is this for" = the **full derived roster at creation time**, not the connected set; unreachable members are *pending deliveries*, never non-recipients; reachable vs unreachable is a delivery state, never a destination state. Item 1 left `temporarilyDisconnectedFingerprints` as exactly "pending deliveries"; item 6 left `handoffTargets` as the custodian set. Design it deliberately; P5's routed store consumes it. Then 9 (prereqs 2, 3, 5, 7 all met; 2 iterations), then 10.
