...
| `Fernlet/FernletKit/Sources/FernletUI/CaptureProtection.swift` | Screen-capture **friction** (never a security control — Docs/Design-Capture-Protection-2026-08-10.md): `CaptureProtectionState` (injected `@Observable` owning the screenshot pulse + per-window-scene `isCaptured` verdict) and `CaptureProtectedModifier` / `captureProtected(surface:)` (opaque Tier-2 cover while recorded/mirrored or scene-inactive; Tier-1 blur + once-per-session nudge after a screenshot). Applied by the app target at the six Private-tab surfaces: the hub root (inner to the lock gate) and the five sensitive sheets. |
...
| `Fernlet/FernletTests/CaptureProtectionTests.swift` | Capture-friction tests: `CaptureProtectionState` transitions off the real notifications (screenshot pulse, per-screen capture re-read, once-per-session nudge claim, teardown without a retain cycle) plus view-level hosting of `captureProtected(surface:)` (opaque cover renders while captured / scene-inactive and clears, pixel-sampled; frontmost gating of the pulse). |
