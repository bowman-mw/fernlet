...
| `Fernlet/App/Fernlet/PrivacyDataSettingsView.swift` | Privacy and data settings screen for managing storage preferences, HealthKit toggles, and sealed backup options. Routes every destructive/OFF toggle through a pre-commit warning; surfaces the sealed-backup restore status + escrow-conflict banner (retry / re-link). Also carries the own-photo escrow switch (with its size disclosure) and the step-5c "Lock photos to this device" consent ceremony — the branch that lets a user device-bind their own photos without turning the iCloud photo backup on. |
