...
| `Fernlet/FernletKit/Sources/PrivateMediaStore/OwnPhotoKeyMigration.swift` | The own-photo key migration: `OwnPhotoCorpusLayout` (the one home of the own corpora's on-disk names), `OwnPhotoKeyMigrator` (eager, idempotent, crash-safe re-seal pass run once per launch off the main path), and `OwnPhotoMigrationLatch` (the persisted, fail-closed proof that gates device-binding and the removal of the read-path dual-open fallback — narrowed by crypto-standardization Phase 3 to "nothing this build can OPEN is left under the old key", since a genuine pre-split file is now unopenable residue the pass steps over). |
