...
| `Fernlet/Fernlet/UITestSupport.swift` | DEBUG-only launch hooks for UI tests: central reader of the `FERNLET_UI_TEST_*` appearance-test flags (seed demo, bypass private lock, jump-to-screen), plus `isTestHarnessActive` (XCTest/UI-test detection that suppresses the backup-exclusion launch gate under automation). |
...
| `Fernlet/Fernlet/TrainerExportView.swift` | `TrainerExportView` — trainer/nutritionist export review + consent screen: pick optional includes and see exactly what is always/never shared before preparing the file. Reached from the **Move tab**'s header "Share" pill (which replaced "Suggest" — that moved into `WorkoutPlanSheet`), not Settings. With `settings.coachExchangeEnabled` on it also carries the manual coach exchange: copy-for-an-AI and paste-a-plan-back. |
| `Fernlet/Fernlet/CoachExportPromptBuilder.swift` | Builds the clipboard blob for the manual coach handoff: instruction preamble + the exact `CoachPlan` JSON schema + Fernlet's enum vocabulary, followed by the windowed `TrainerExportBundle`. No exercise catalog is sent. |
| `Fernlet/Fernlet/CoachPlanImporter.swift` | Pasted text → `CoachPlan` → dated `PlannedWorkout` rows. Fail-closed JSON extraction/decode, `WorkoutSafetyFilter` pass, collision policy (spec D4), new-exercise registration, audit event. Also resolves `CoachPlanEdit`s (`ResolvedCoachPlanEdit`) that adjust / replace / delete workouts the user ALREADY planned, targeted by the ids the export echoes — refusing anything logged, missing, or in the past. |
| `Fernlet/Fernlet/CoachPlanPasteSheet.swift` | `CoachPlanPasteSheet` — where a plan comes back in: `TextEditor` + system `PasteButton` (never a programmatic `UIPasteboard` read), decode only, no writes. |
| `Fernlet/Fernlet/CoachPlanReviewView.swift` | `CoachPlanReviewView` — the review gate: start-date picker, blocking issues, per-exercise safety strikes, new-exercise list, collision choice, day-by-day preview. Nothing is written until Accept. |
...
| `Fernlet/Fernlet/TrainerExportBuilder.swift` | `TrainerExportBundle` / `TrainerExportOptions` / `TrainerExportWindow` + `FernletStore` builder — curated fail-closed-allowlist workout + nutrition export for a trainer/nutritionist, with opt-in extras. Also carries macro targets, training setup (location/equipment/split), and the per-exercise progression rollup (`ExerciseLineParser`). The window defaults to unlimited (file/mesh) and narrows for the clipboard. |
...
| `Fernlet/FernletUITests/CaptureProtectionUITests.swift` | Asserts the forced Tier-2 capture cover (`FERNLET_UI_TEST_FORCE_CAPTURE`) and its accessibility label render over each of the six capture-protected surfaces; the two JournalView-owned sheets are auto-presented via their DEBUG launch hooks because the cover blocks navigation taps. |
