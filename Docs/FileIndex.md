...
| `Docs/Crypto-Domain-Separation.md` | The cryptographic domain registry: the full 47-entry inventory across five families, the rule that purpose spellings are FROZEN protocol data (never localized, never assembled from input, never casually renamed), transcript framing and the `91c3956` mismatch it records, which formats are version-marked v2/v3 and what still reads older bytes, the procedure and review checklist for adding a domain, and — at length — what the two boundary suites can and cannot catch, including the 18 `// cryptographic-domain:` escape-hatch sites nothing else tracks. Backed by `CryptographicPurposeBoundaryTests` (a purpose is USED) and `CryptographicDomainSeparationTests` (the registry's values are USABLE for separation). |
