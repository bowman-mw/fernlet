...
**Written:** 2026-09-18, at the P7 boundary (`main` = the P7 close-out, fast-forwarded from `claude/zen-goodall-wejlxg` at the owner's request; **P7 is on `main` and UNBUILT** — no P7 file had compiled when it was merged).
...
| # | Item | Tier | Prereq |
|---|---|---|---|
| 0 | **Build and run P7 — the gauntlet the P7 session could not.** In the P7 ledger's order (§ "Owed to a Mac"): the two scans; `build-for-testing`; `ProximityRunPolicyTests` alone as the warm-up; per item, the named suites and the **red-once** step for every wall (W8's two halves; the retirement wall by verb and by name; the poller wall; the resume surface wall; `everyMeshAcceptanceBatteryIsGated` by deleting one selector); the mesh-batteries line exactly as the workflow spells it and **the floor raised to what it executed** (expected 313; write the measured number, and the same for `CIGateSelectorBoundaryTests`' pin, 42 by declaration); the **catalog sync** of P7's thirteen sentences from `HEAD`'s blob (count them against the ledger's list); the UI suite serially; the full suite against P6's baseline (4 866 / 486, one invocation, EXIT=0). Every red is a **P7 fix commit** with its own ledger line; the watch list is in the ledger (Release-config main-actor isolation vs `ProximityRunPolicy.Input`'s `Hashable`; the funnel's `duressSessionActive` parameter shadowing; a real `FernletStore` per funnel cell). Then the three simulator eyeballs the ledger names (Friends tab entry/exit; Control Centre over the Friends tab; delete-all during a live session). **Item 0 ends with plan §13's heading changed from IMPLEMENTED, UNBUILT to BUILT, with the measured numbers written into §13.4** — one commit, one ledger line. **Two device findings from the owner's first phone ↔ Simulator run (2026-09-18, on `main` before P7 was merged) belong to item 0's fix list, each as its own commit with a red-once cell:** (a) **the asymmetric-commit founding outage** — without UWB the two proximity commits are two taps seconds apart; the first tapper founds and sends its descriptor at once, the second tapper's slot is still uncommitted so the door drops it (`mesh.meshDescriptor.droppedUncommittedSlot`) and nothing re-sends it; the second tapper then founds its own mesh, and the pair merges only if the first tapper has the HIGHER fingerprint (it yields to the second's descriptor) — otherwise the first tapper drops the second's as foreign (`mesh.descriptor.droppedForeignMesh`) and both sit alone forever (chat `.noDestinations`, photos silent). `MeshFoundingRig` never saw it because `commit(0,1); commit(1,0)` both run before `settle` delivers a frame. Fix: on the merge door's foreign refusal, when the sender is this device's only committed peer, the incoming mesh has one member and `foundsPairwiseMesh` says this device survives, re-send this device's descriptor to that slot (bounded: once per refused descriptor); the peer then yields through the existing path. Cell: commit one side, `settle`, commit the other, `settle` — red today. (b) **the recipe listener's self-stop has no re-start under the edge-triggered seam** — `ProximityRecipeShareManager.onTransportError` stops itself on a `didNotStart*` (the Local Network permission prompt interrupts the very first start) and its doc relies on the view calling `start()` on the next tab / scene / lock change; P7's `ProximityRunTransition` re-applies a listener only on a verdict CHANGE, and the prompt's inactive → active round trip is not one, so the listener stays dark until the share sheet's own `start()` — the owner's observed workaround. Fix: make the two listener seams level-triggered (re-apply `start()` / `stop()` to presence and recipe share on every policy run; both `start()`s guard on their running flag), keep the mesh seam edge-triggered, and re-pin `ProximityRunSeamsTests`' "unchanged verdict is silent" cell to the mesh radio only. | 1 + 1b | — |
...
**Started:** 2026-__-__ · **Iteration:** 1 · **Tree at seed:** `main` = the P7 close-out (fast-forwarded 2026-09-18); P7 UNBUILT until item 0
