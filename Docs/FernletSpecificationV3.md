...
- **Enabling the encrypted period backup defers rather than fails.** Its payload is sealed under the `.privateHub` content key, but the toggle lives in Privacy & Data, reached from the Home tab — where the hub is always re-locked. So `setSealedBackupEnabled(true, payloadType: .periodData)` treats `SealedBackupWiringError.locked` as a deferral: the preference sticks, `sealedBackupPeriodReuploadDeferred` is set (already surfaced by the Privacy & Data status banner), and `retryDeferredPeriodReuploadIfNeeded()` seals it the next time the Private tab unlocks — or at the next launch, via the pre-existing launch pass. Disabling never takes this path; it deletes the cloud chunk set and needs no content key.
