...
5. **Two smaller replay/digest residuals, both named, neither closed.** (i) D-12.15: a custody
   ...
   **Cost:** staleness, never a lost delivery; a cross-device un-record would be a wire change.
   ...
   now refuses a digest whose signed `sentAt` is strictly before the recorded one
   (`MeshRoutedInventoryStampRule`, one audit line `mesh.routedInventory.staleSentAt`, no
   refusal-budget charge — the digest family stays outside that door per D-5.12/D-6.10), and the
   property battery's thirteenth claim asserts `inventorySentAt` never moves backwards at any member
   for any peer. An equal stamp is still admitted silently: an idempotent replay re-records the same
   value. The text below is what the residual said before it was taken.
   ...
   The guard was handed forward because it changes items 5/6's door and the stamp item 7's window rule
