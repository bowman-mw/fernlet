...
| **D4** | New plan arrives while one is active | **Prompt** — replace remaining days or keep current; never rewrite completed days. |
| **D5** | Reusing open-source FernletKit modules inside closed-source Coach | Needs a license check/choice for the Fernlet repo (permissive or dual-license the shared kit). Blocking for Coach development start. |
| **D6** | Universal-link domain + preview card | One domain (e.g. `fernlet.app`), static AASA (app + appclips entries) + per-length static pages ("7-Day Workout Plan"). No edge server. Per-plan titles: see D11. |
| **D7** | Coach app business model / App Store positioning | Out of scope for this spec; note that Coach being paid-up-front fits "closed-source, no accounts, no subscriptions infra." |
| **D8** | Coach multi-device | **Single device V1** + sealed backup file. Multi-device would need the mesh-offline-sync work already on the Fernlet roadmap. |
| **D9** | Error copy | Requirement says "Incorrect message sent". Recommend shipping copy: **"This workout was sent to someone else"** + explanation; keep the requirement's phrase as the accepted meaning, not necessarily the literal string. |
| **D10** | Sharing a session note with its trainee (in person) | Not in V1; tracked as a candidate follow-up. |
