...
**Started:** 2026-09-19 · **Iteration:** 3 (item 2 pass 1, 2026-09-19) · **Tree at seed:** `main` = the P8 close-out (`bb454fe`); P8 BUILT at tier 1/1b, §15 NOT RUN; the P7 lane fix `80934b7` is on `main` (HEAD~1 at seed)
...
| 2 | Presence over QUIC — posture first | 1 + 2 | — | in-flight (pass 1 done) | `d7342f3` (pass 1) | **Pass 1 DONE:** `PresenceEpochPosture` (pure value; `fn-` + 8 CSPRNG bytes hex; identity via `EphemeralMeshTLSIdentity.mint` at the **epoch START**), wall-clock-anchored epochs, the seam `PresenceManager.presencePosture` on the existing tick; 9 + 4 cells; nothing over the air. Verify → FIX FIRST (2 fixes, 7 notes, all applied): **the certificate's validity window encoded the mint SECOND** — a mid-epoch start was singled out of the anonymity set (fixed: minted at `presenceEpochStart`); docs overstated the shipping property; mint retry budgeted once per epoch. Gates: 293 / 21 green, wall PASSED, three red-onces. **Pass 2 next:** `_fernlet-near2._udp` over `NetworkMeshSession` in a new `NetworkPresenceSession.swift` (the no-tracking wall's `permittedLocalLinkFiles` + `Info.plist` `NSBonjourServices` + `Docs/No-Tracking-Wall.md` in the same commit), the MC path gone from `PresenceManager`, the tier-2 rotation row |
