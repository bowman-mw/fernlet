...
Scope: friend mesh for nearby photo and attachment sharing, plus an asymmetric trainer-led mesh for classes. Builds on existing `MultipeerSession`, `ProximityCoordinator`, `IdentityService`, and `FernletStore`.

This document tells the implementer exactly what to build and what not to build. Anything ambiguous below is intentionally left open; ask before guessing.
...
Two phases. Phase 1 ships a working friend mesh. Phase 2 adds polish and the deferred friend-of-friend feature.
...
- **Trainer mesh routing** — instructor-side fan-out, client-side ignore-other-clients enforcement. New trainer mesh UI deferred separately.
- **Friend-of-friend labels via cached vouches** — at handshake completion, optionally exchange "currently-trusted fingerprints" lists, cache per friend. Use these caches to label strangers in meshes when their mutual friend is absent.
- **Overflow eviction** — when at 5 slots and a closer peer is discovered, evict the most-distant lightweight slot.
- **Per-sender quota on send (10 photos max per mesh session)** at the UI level.
