...
| Path | Where | Notes |
|---|---|---|
| Recipe web import | `AIProviders/RecipeWebImporter.swift` (`fetchHTML`) | Fetches the URL the user pasted or shared into the app. HTTPS-only, SSRF-guarded on the initial URL *and* every redirect hop (`isSafePublicHTTPSURL`), 3 MB bounded, **15 s idle / 120 s whole-transfer** bounded (2026-08-18: the caller's `URLRequest(timeoutInterval: 15)` is an *inactivity* timer a trickling server resets forever, so the whole-transfer ceiling lives on the shared session as `EphemeralWebSession.maxResourceSeconds`). |
| Recipe image download | `AIProviders/RecipeWebImporter.swift` (`downloadImage`), called only from `App/Fernlet/FernletStore.swift` (`fetchRecipeWebImageIfNeeded`) | One GET for the recipe page's own main food picture (a host the *page* named — JSON-LD `image` / `og:image`), which becomes the recipe's default photo, sealed through the private media store. **Owner decision 2026-08-09, consciously reversing the 2026-07-16 "no external image fetch" tester decision.** Runs on USER-PRESENT paths only — the foreground paste-a-URL import, the first open of a recipe's detail, and the post-"Re-import from source" refresh — never the share-extension background queue drain. **One attempt per device, suppression syncs**: each device gets one automatic attempt (device-local `RecipeWebImageAttemptMemory` sidecar, re-armed only by the explicit re-import; cancellation by navigating away does not spend it), while deleting the photo — or the user already having their own — sets the synced `webImageSuppressed`, pinning the recipe photo-less on every device. Same guard rigor as the page fetch: SSRF-checked on the initial URL and every redirect hop (IP literals canonicalized, so decimal/hex/octal/IPv4-mapped spellings classify like their dotted form), an image MIME requirement (`image/*`, or a generic octet-stream declaration whose bytes must pass a magic-number image sniff — mislabeled image CDNs work, HTML error pages fail), 15 s idle / 120 s whole-transfer timeout, 10 MB streaming cap that aborts oversize. Proximity recipe shares carry a downscaled copy of the picture *in the sealed payload*, so a receiving device performs **no** web fetch. |
| Product page import | `App/Fernlet/FoodProductWebImporter.swift` (`fetchHTML`, `fetchImage`) | Fetches the product page the user chose (or the page the allowlisted search returned), plus candidate nutrition-label images for OCR. **SSRF parity with the recipe fetch (2026-08-18):** `fetchHTML` was scheme-only with no redirect delegate; it now runs `RecipeWebImporter.isSafePublicHTTPSURL` on the initial URL and re-validates every redirect hop with the shared `RecipeWebImporter.RedirectValidator`, and the DuckDuckGo result unwrap (`resultURL`) applies the same predicate so a private-literal result is never even surfaced as an openable preview. `fetchImage` routes through `RecipeWebImporter.downloadImage` (2026-08-09), so images get the full SSRF/redirect/MIME guard, 12 MB streaming cap, and the bytes are then pixel-probed and thumbnail-decoded before OCR (2026-08-18) so a few-hundred-KB image declaring 256 MP cannot materialise a bitmap the byte cap never bounded. 15 s idle / 120 s whole-transfer timeout. **Honest limit, shared with the recipe fetch:** the SSRF predicate rejects private *literals* in every spelling, not a public hostname whose DNS answer is private — neither importer resists DNS rebinding. |
| Source-link connection pre-warm | `App/Fernlet/FoodView.swift` (`SourceLinkPrewarmModifier`) | `SFSafariViewController.prewarmConnections(to:)` on the recipe detail page and the saved-recipe notes sheet: the recipe's own https source host is now contacted **on detail-appear** — a DNS lookup + TLS handshake for the eventual Safari presentation, no HTTP request, no page load (**owner decision 2026-08-09**: pre-warm only, no snapshot, no `WKWebView`; `SFSafariViewController` stays the opening surface). The token is invalidated on disappear. Like the Safari presentation itself (§2a exception), this runs out of process and outside the private-tab session; it reveals only "a Fernlet user who saved this recipe opened its page" to a host the user already chose by importing from it. **Peer-received recipes are excluded** (2026-08-18): a recipe imported over the proximity mesh carries a source URL a *stranger* chose, where that justification is simply false — a unique per-recipient hostname would turn every open of that recipe into a DNS+TLS beacon back to the sender. The mesh import path stamps `RecipeWebImport.sourceIsPeerSupplied = true` and `SourceLinkPrewarmModifier.prewarmURL(for:)` returns nil for such a recipe, so the pre-warm never runs for it; an explicit tap on the source link (or on "Re-import from source", which now names the host in its confirmation) stays the consent point. Recipes imported over the mesh *before* this flag existed decode it as nil and keep pre-warming — there is no reliable retroactive signal, and the flag is additive/tolerant-decoded. No permitted destination is added or removed by any of this. Repeat imports of an already-saved URL now skip the network entirely (normalized source-URL match), so this pre-warm is paired with strictly *less* re-fetching, not more. |
...
| **Network.framework / QUIC** — **the friend mesh's transport**, since the cutover of 2026-09-21 (the factory it flipped left the tree with the MultipeerConnectivity radio on 2026-09-22; there is one radio now and nothing selects it); migrated per [the ProximityKit network migration](Plan-ProximityKit-Network-Migration-2026-08-27.md) §7 | `ProximityKit/Transport/NetworkMeshSession.swift` | `_fernlet-mesh2._udp` |
...
- **The heart dead-drop uses the CloudKit *public* database**, and public-DB records carry a
  ...
  *some* iCloud user wrote N records on a given day. They cannot see who, to whom, or what: tags are
  pseudonymous, rotate daily, are uncorrelatable across days, and payloads are sealed and padded to a
  size bucket. Two things that residual *does* include, stated so it is not read as smaller than it
  is. First, the creator id is **stable per container**: the tags don't link across days, but the
  account does, so what a dashboard viewer really has is one anonymous account's send-activity
  timeline — and its distinct tags on a day equal how many different friends it wrote to that day.
  Second, **cleanup is client-side and creator-only**: only the device that wrote a record can delete
  it, from record names held in its own local outbox, and there is no server-side TTL. Deleting the
  app without running Delete Everything — or a delete-everything whose remote purge couldn't run, the
  case the app itself names as "hearts parked in iCloud" — leaves those sealed records in the public
  database indefinitely, unopenable by anyone but the intended recipient and deletable by nobody.
  This is a documented, accepted residual (`HeartDropCloudTransport.swift`), not an oversight, and it
  is the one place where a developer-visible byte exists at all.
...
- **Apple frameworks are trusted, not audited.** CloudKit, WeatherKit, and APNs make network calls we
  ...
  capability this gives the *service operator*, which is a different and larger one than the dashboard
  view described above: CloudKit necessarily sees which account queried which tag, so a tag that was
  actually written links the writer's account to the reader's — Apple-side only, never
  developer-side.
