...
| HealthKit residue cache — this device's HealthKit readings per day (the activity / body / mindfulness context, the sleep hours HealthKit wrote into a sleep log, and workouts imported from Apple Health), kept here since 2026-09-23 because the storage strip removes every one of them from the synced rows and blob; plus its one-time legacy-scrub marker. Cycle and intimate groups are never in it | `Application Support/FernletDeviceHealth/health-residue.json` (`FileDeviceHealthResidueStore`, device-local, backup-excluded, complete file protection, ≤ 400 days) | `deviceHealthResidueStore.clearAll` (in `resetAll`, so both wipe paths cover it; the HealthKit opt-out's `CoreDataHealthKitCacheCleaner` empties the same instance) |
