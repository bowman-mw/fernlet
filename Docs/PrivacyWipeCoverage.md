...
| Own-photo escrow backup — every sealed photo body **and** its corpus manifest, for meal / recipe / progress; the same call also clears the device-local upload ledger (`OwnPhotoUploadLedger`, UserDefaults `fernlet.sealedPhoto.uploadedIDs.*` — photo IDs only, never bytes or captions), so a torn-down backup leaves no claim of ownership behind | CloudKit private DB (`SealedPhotoRecord`) + UserDefaults | `deleteOwnPhotoEscrowBackups` (runs BEFORE the preference reset that would gate it off, and needs no escrow key — deletion is by record name, so it works while locked) |
