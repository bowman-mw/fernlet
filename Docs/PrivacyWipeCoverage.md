...
| Sealed-column format-migration latch (crypto-standardization Phase 2.6): `SealedColumnMigrationLatch`, `com.fernlet.private-store.sealedColumnMigrationComplete` — one bit, no content. Cleared because the same `sealedStoreRebuildHook` closure destroys the latch's entire subject — the four sealed entities' seven ciphertext columns — and the funnel tolerates a rebuild failure (`(try? …) != nil` feeds the outcome), so a kept latch could stand over rows a failed purge left behind: the `pendingNarrativeBufferMigrationComplete` row's reasoning, applied to the store the rows live in; contrast `ownPhotoKeyMigrationComplete`, kept because its subject — the re-sealed local files — survives the wipe. `FernletLockService.reset()` (the destructive lock reset, which purges and rebuilds the same store inside `FernletLock`, never through this funnel) clears the same latch beside its own purge for the same reason. The next `.privateHub` unlock's keyless revalidation census re-proves over the empty store and a clean pass re-latches | UserDefaults (device-local, never synced) | `SealedColumnFormatMigrator.latch().reset` — the FIRST statement inside the `sealedStoreRebuildHook` closure, before `rebuildStore()`, so even a failed rebuild cannot leave a stale proof standing |
...
| Locked-note pending buffer; plus the Phase 2.4 buffer-format migration latch (`PendingNarrativeBufferMigrationLatch`, `com.fernlet.private-store.pendingNarrativeBufferMigrationComplete` — one bit, no content), cleared because this same closure destroys the latch's entire subject — the buffer file — and the funnel tolerates a purge failure (`(try? …) != nil` feeds the outcome), so a kept latch would keep claiming clean over a file the purge failed to remove: the same case as `fernlet.sealedPhoto.hashVersionMigrationComplete`'s and `com.fernlet.heartdrop.sidecarFormatMigrationComplete`'s cleared rows (subject destroyed); contrast `ownPhotoKeyMigrationComplete`, kept because its subject — the re-sealed local files — survives the wipe. The next launch re-proves with one census take, which post-wipe reads `.absent` — an *earned* zero on this transient surface, since the buffer's drain-and-purge lifecycle makes absence real evidence and `saveEntries` can only re-create the file in v2 | PendingNarrativeBuffer + UserDefaults | `pendingNarrativeBufferPurgeHook`, whose closure also runs `PendingNarrativeBufferFormatMigrator.latch().reset` for the latch bit — cleared FIRST inside the closure, so even a failed purge cannot leave a stale proof standing |
...
| Away-hearts drop state: one-time + signed prekeys (keychain `com.fernlet.heartdrop`, incl. the `sidecarSealKey` sidecar seal key — `deleteAll` is by service, all accounts), peer bundle cache, outbox **and its `HeartDropOutbox.json.corrupt` quarantine file**, durable dedup, service identity cache; plus the Phase 2.2 sidecar-format migration latch (`HeartDropSidecarMigrationLatch`, `com.fernlet.heartdrop.sidecarFormatMigrationComplete` — one bit, no content), cleared because this same funnel destroys the latch's entire subject — the sidecar files and the seal key — so the recorded proof would predate the wipe: the deliberate mirror-image of `ownPhotoKeyMigrationComplete`'s kept row (its subject, the re-sealed local files, survives), and the same case as `fernlet.sealedPhoto.hashVersionMigrationComplete`'s cleared row (subject destroyed); the next launch re-proves with a four-stat marker survey | Keychain + sidecars + UserDefaults | `heartDropService.wipeForDeleteAll` (each sidecar's `ProtectedSidecar.wipe()` removes primary + quarantine paths), then `HeartDropSidecarMigrationLatch.resetForDeleteAll` for the latch bit |
...
| `unresolved:column.attributeName@FernletKit/Sources/PrivateStoreCore/SealedColumnFormatMigration.swift` | Not a defaults key at all: `row.setValue(newBlob, forKey: column.attributeName)` — the Phase 2.6 sealed-column format migrator re-sealing one ciphertext column of a sealed **Core Data** row (the census's seven attribute names) | Recorded as `unreachableByDesign`: the KVC spelling is what the discovery scan anchors on, but the receiver is an `NSManagedObject` of the `FernletPrivate` store, which the funnel empties and destroys under its own cleared rows (`sealedStoreRebuildHook` + the sealed row-delete hooks) |
| `unresolved:entry.column.attributeName@FernletKit/Sources/PrivateStoreCore/SealedColumnFormatMigration.swift` | Not a defaults key: the same migrator's compensating restore writing a sealed Core Data column's held old bytes back | Recorded as `unreachableByDesign`, same substrate and same funnel coverage as the sibling seam above |
