...
| **Own-photo device-binding consent** — `UserDefaults` `com.fernlet.private-media.ownPhotoDeviceBindingConsent` (device-local, non-synced) | One bit: "the user accepted that their own photos are locked to this device" (step 5c). Kept for the same reason the row above is: the wipe empties the photo stores but leaves the key, so clearing the consent would silently *widen* custody for everything captured after the wipe — the next launch would find the gate unsatisfied and photos would go back to being backup-restorable without anyone deciding that. It records a custody preference, never content: no timestamps, no counts, nothing about any photo. (It is also one-way by design — nothing in the app withdraws it, because un-binding is a security regression the user could trigger by accident.) The migration latch beside it, `com.fernlet.private-media.ownPhotoKeyMigrationComplete`, is kept for the mirror-image reason: nothing the wipe leaves behind is under the pre-split key, and clearing it would only force a pointless re-scan. (Stated that way since the crypto standardization round's Phase 3, which narrowed what the latch attests: it used to mean "every own file was re-sealed under the own key", and now means "no own file this build can open is still under the old key" — a genuine pre-split file is unopenable residue the pass steps over rather than heals. The keep decision is unaffected: the wipe empties the own corpora, so a post-wipe pass would latch trivially either way) | Dies with the app container on uninstall / device reset |
