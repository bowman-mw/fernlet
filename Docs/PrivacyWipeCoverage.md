...
| Sealed iCloud backups (all payload types — period, **journal narratives**, **intimacy logs**, and any surviving copy of the **retired** sensitive-notes payload, the old Tier-2 backup: since 2026-09-23 it is never uploaded, the sealed-backup launch pass deletes it quietly, and until that delete lands its `sealedBackupSensitiveNotesEnabled` marker keeps it targetable here) | CloudKit private DB | `setSealedBackupEnabled` |
...
| Messages App Group catalog — the bounded document the app PUBLISHES for the Messages extension: prebuilt recipe and workout-plan packets with their picker metadata, up to 100 of each. It lives in the same App Group and is readable by a process Messages hosts, so a surviving catalog leaves the composer offering recipes from a store the user just emptied. Cleared in leg 10 alongside the widget mirror, and for the same ordering reason: it runs AFTER the funnel's second `cancelPending`, so no debounced save can re-publish it moments later. **Unconditional since 2026-09-23:** it used to run only when this launch had activated the publisher, but the file on disk is whatever a PREVIOUS launch published, and a duress wipe fired at the lock screen can run before launch wiring reaches `activateMessagesCatalog()` — so exactly that wipe left the catalog behind while reporting itself complete. The funnel now builds a publisher over the same directory when none is live; `DeleteAllDataTests.messagesCatalogIsClearedEvenWhenThisLaunchNeverActivatedIt` pins it | App group | `messagesCatalogPublisher.clear` (a failure appends "Messages catalog" to the incomplete-stores list, so a surviving file cannot leave the dialog claiming a complete wipe) |
...
| Session temp messages — the live-session chat transcript. Dropped in **leg 0**, at the top of the funnel, through the manager's ONE clear funnel: that is what bumps `transcriptGeneration` **and** what switches the routed projection off for the rest of the wipe. Both halves matter, because the sealed routed store beneath the transcript is not destroyed until leg 11, and every suspension point in between was a window in which a rising access edge re-projected the just-wiped messages into a transcript that is still live, in the same mesh (P6 item 4 fix review). Leg 7b's old direct `sessionMessages.clear()` did neither and is gone. `endPrivacyWipe()` runs from a `defer`, so no exit leaves the projection switched off for the process | Memory (SessionMessageStore) | `meshNetworkManager.beginPrivacyWipe` |
| Every proximity radio — the mesh session, the friend search and both listeners (presence, recipe share) — stood down at **leg 0** under the raised `deleteAllInProgress` fact: the run policy (network migration P7) answers `stop` for every radio and its seams execute it, which replaced the direct presence stop leg 7b used to hold | PresenceManager / MeshNetworkManager / ProximityRecipeShareManager | `reapplyProximityRunPolicy` |
...
| HealthKit residue cache — this device's HealthKit readings per day (the activity / body / mindfulness context, the sleep hours HealthKit wrote into a sleep log, and workouts imported from Apple Health), kept here since 2026-09-23 because the storage strip removes every one of them from the synced rows and blob; the body profile this device imported from Apple Health (age, sex, height, weight — `DeviceHealthBodyProfile`, which used to overwrite the synced `settings.userProfile`; `resetDiary` also drops its in-memory copy); plus its one-time legacy-scrub marker. Cycle and intimate groups are never in it | `Application Support/FernletDeviceHealth/health-residue.json` (`FileDeviceHealthResidueStore`, device-local, backup-excluded, complete file protection, ≤ 400 days) | `deviceHealthResidueStore.clearAll` (in `resetAll`, so both wipe paths cover it; the HealthKit opt-out's `CoreDataHealthKitCacheCleaner` empties the same instance) |
...
| Day rows + blob, the local JSON day-blob FILE, **including each meal component's bind score and the web-nutrition consent/revocation decision**, the **device-local Tier-2 memory sidecar** (`FernletDatabase-TierTwoMemories.json` beside the local JSON file — `TierTwoMemoryStore`; since the 2026-09-23 owner decision Tier-2 is never in the mirrored blob, and the sidecar is excluded from device backups unconditionally), and the pre-database `LegacyKeys` UserDefaults corpus — `fernlet-settings`, `fernlet-recent-meals`, `fernlet-previous-journals`, `fernlet-memories`, `fernlet-goals`, `fernlet-workshop`, and every interpolated `fernlet-day-<yyyy-MM-dd>` row, which holds journal + memory JSON UNSEALED in the preferences plist | Core Data/CloudKit/disk + UserDefaults (`.standard`) | `repository.purgeAllPersistedData` → `LocalFernletRepository.clearLegacyUserDefaultsIfPresent()` and `TierTwoMemoryStore.purge()`, both run BEFORE the file-existence guard so the shipping Core Data configuration (which reaches the local repository with no JSON file at all) still clears them; `CoreDataFernletRepository` also purges the sidecar itself. Left behind, the legacy keys both survive the wipe as plaintext AND re-hydrate the store on the next launch, because an absent database file is indistinguishable from a first launch and routes straight through the legacy migration |
...
| Sealed mesh-session context (P3 item 2, plan §8.1): `MeshSessionContext.sealed` + its `.corrupt` quarantine sibling in `Application Support/Fernlet`, **and** the key that seals them (keychain service `com.fernlet.mesh-session`, account `meshSessionContextKey`, `AfterFirstUnlockThisDeviceOnly`; `deleteAll` is by service, all accounts). Holds the mesh id, `createdAt`/`hardDeadline`, the signed admission/departure/removal/termination records a roster is derived from — i.e. who the user was in a room with — and, since P6 item 1 (schema 3), each admitted member's **own signed key-agreement public key** plus the fingerprints whose key this device refuses to resolve. Those two additions are fields inside the same sealed file, not a new surface: no new file, no new keychain account, no new `UserDefaults` key, and no new disposition row. They are public keys of admitted members, which is the same class of fact as the roster they sit beside. Since owner-calls item 3 (2026-09-22) it also carries **`endingPresented`**, one Bool: whether the person has already been shown this context's ending on the Friends tab, so the "previous session ended" card is told once instead of at every cold start. Same file, same key, same wipe — a field, not a surface, and deliberately not a `UserDefaults` key: it must die with the context it describes (delete-all, or a new session's context replacing it) and must not outlive the rejoin bar the same file carries. The group control key is **not** in it and is never persisted anywhere | Sealed file + keychain | `MeshSessionStore.wipeForDeleteAll` — one call takes file and key together, by design: ciphertext whose key survives is unreadable noise, and a key whose file survives is a promise to decrypt it |
| Sealed routed-content store (P5 item 3, plan §11/§19.5): `MeshRoutedIndex.sealed` + its `.corrupt` quarantine sibling + every `MeshRoutedChunks/<uuid>.chunk` payload file in `Application Support/Fernlet`, **and** the key that seals them (keychain service `com.fernlet.mesh-routed`, account `meshRoutedStoreKey`, `AfterFirstUnlockThisDeviceOnly`; `deleteAll` is by service, all accounts). Holds routed ciphertext this device is holding for other people — friend-wall photos, and since P6 item 4 **live-session chat messages too**, which is the one thing this row's wording did not already cover: the *visible* transcript is still memory-only and still vanishes at session end, and the ciphertext beneath it does not, so a message is one of the things this store's wipe destroys — the `MeshCustodyReceipt` facts it has signed, the receipts other members signed for it, the per-item delivery map — which destination fingerprints this user was sending what to, and how far each got — and, since P5 item 4, the **final-acknowledgement instants** this device recorded plus the `MeshRecipientReceipt` evidence behind every `delivered` entry, **this device's own receipt included** (it is stored rather than re-minted, because a heart's final-ack condition is a one-shot ledger judgement). Those two additions are fields inside the same sealed index, not a new surface: no new file, no new keychain account, no new `UserDefaults` key, and no new disposition row. The per-item content key is wrapped per recipient inside the origin's signed manifest and is never a group secret, and this store never unwraps one | Sealed files + keychain | `MeshRoutedStore.wipeForDeleteAll` — one call takes files and key together, by design: ciphertext whose key survives is unreadable noise, and a key whose files survive is a promise to decrypt them |
...
| **Own-photo device-binding consent** — `UserDefaults` `com.fernlet.private-media.ownPhotoDeviceBindingConsent` (device-local, non-synced) | One bit: "the user accepted that their own photos are locked to this device" (step 5c). Kept for the same reason the row above is: the wipe empties the photo stores but leaves the key, so clearing the consent would silently *widen* custody for everything captured after the wipe — the next launch would find the gate unsatisfied and photos would go back to being backup-restorable without anyone deciding that. It records a custody preference, never content: no timestamps, no counts, nothing about any photo. (It is also one-way by design — nothing in the app withdraws it, because un-binding is a security regression the user could trigger by accident.) The migration latch that sat beside it, `com.fernlet.private-media.ownPhotoKeyMigrationComplete`, is **GONE** — retired with `OwnPhotoKeyMigrator` at the close of the crypto standardization round (owner decision 2026-08-29), so it no longer exists as a persisted surface and carries no disposition row. Its job — proving this device could walk and classify its own photo corpora, which is `OwnPhotoKeyBinder`'s first gate half — passed to `com.fernlet.private-media.mediaAtRestFormatMigrationComplete`, whose kept row is below. Any device that already latched the retired key keeps one orphaned `true` bool that nothing reads; it holds no content and dies with the app container | Dies with the app container on uninstall / device reset |
| **Media at-rest format-migration latch** — `UserDefaults` `com.fernlet.private-media.mediaAtRestFormatMigrationComplete` (device-local, non-synced) | *Added with crypto-standardization Phase 2.3.* One bit: "a full sweep of the eight-location media surface proved no bytes remain that only `gcmOpen`'s legacy-read branch can open" (`MediaAtRestFormatMigrationLatch`). Kept, but NOT on the cleared format latches' "subject destroyed" reasoning nor on the retired `ownPhotoKeyMigrationComplete`'s "files are destroyed" clause — that premise is false for three of its eight locations, because the friend wall (photos, thumbnails, sealed index) deliberately survives this funnel (`deleteAllSessionPhotos` NOT called; the rows above). The sound basis: the own corpora ARE emptied by the wipe, the surviving wall corpus was proven all-current-format (or named residue) before the latch could set, and every post-wipe writer emits the current format — so the latch's claim stays true of everything the wipe leaves behind, and clearing it would only force a pointless re-scan. It records a format fact, never content: no counts, no timestamps, nothing about any photo. (Contrast `fernlet.sealedPhoto.hashVersionMigrationComplete`, cleared because its subject — the cloud manifests — dies in the same call) | Dies with the app container on uninstall / device reset |
| ModerationBanStore self-ban — keychain `com.fernlet.moderation`, account `selfBan.device` | 2026-07-17 decision: a device ban must survive a wipe or a wipe is a ban-evasion tool. The PEER bans co-located in the same service are cleared (cleared-by table above) — the clear is account-prefix-scoped for exactly this reason, so never "simplify" it into a service-wide `deleteAll`. **Since 2026-09-24 the row also carries the ban's evidence** (tracker §3.5): per report, a reporter TAG — SHA-256 over a registered domain, the row's own random salt and the reporter's key — plus the artwork hash and report seq. The tag is what lets a reporter's later withdrawal lift the ban; it is a salted digest precisely so this wipe-surviving row never holds another person's key, and a fresh salt per ban keeps tags unlinkable across bans. The ledger rows that name the reporters are still cleared by `moderationLedger.clearAll` above | Serves out on its credited-time clock (30 days), or LIFTS when the reporters themselves withdraw enough of their reports (`ModerationBanStore.reconcile`; Docs/Moderation-SelfBan-Recovery-2026-09-23.md). Nothing the device does alone — including this wipe — lifts it |
...
| **Ledger reset-boundary sidecars** — `UserDefaults` `fernlet.coinLedger.pendingResetBoundaries`, `fernlet.milestoneLedger.pendingResetBoundaries` (device-local, non-synced; the default store's `PersistenceController.localDefaults`) | *Added 2026-09-24 (tracker §3.6).* The wipe's OWN boundary marker for each append-only ledger — a `reset:<instant>` id, the wipe's day and instant, no content — held here from BEFORE the ledger's rows are deleted until the same marker has landed in the synced store. Every load merges it into the ledger and retries its append, so a process death between a failed marker append and its retry can no longer let rows another signed-in device syncs back count again. It is written BY this funnel (`coinLedgerService.reset` / `milestoneLedgerService.reset`), so a clear in the same funnel would erase the only copy that outlives the process — the opposite of the wipe's intent. It discloses nothing the synced marker itself does not keep forever by design | Retired by the ledger the moment the synced marker lands — on a healthy store, inside the same wipe call; otherwise at the next launch that can write. Dies with the app container on uninstall |
...
| **Deferred lock setup** — `UserDefaults` `lockSetupDeferred` (`OnboardingDefaults`) | *Added 2026-08-20 by the discovery wall, which found it in neither table.* One bit: the user chose "not now" at the onboarding app-lock step, so lockable features may prompt later — since 2026-09-24 the progress-photo strip's one-time nudge (`DeferredLockSetupNudge`, next row) is the prompt that reads it. The app lock itself survives this funnel **by design** (first row of this table), so clearing this bit would re-nag about setting up a lock the device may already have. It records a UI decision, never content | Cleared by actually setting up the lock — at the onboarding lock step or from the progress-photo nudge; dies with the app container |
| **Progress-photo lock-nudge answer** — `UserDefaults` `fernlet.progressPhotos.lockNudgeAnswered` (`OnboardingDefaults`) | *Added 2026-09-24 with the nudge.* One bit: the user answered the progress-photo lock-setup nudge — "Not now", or a lock set up from it — so it never shows again. Same class as the deferral bit above: a UI decision, no content, no timestamps, nothing about any photo. The app lock and the deferral both survive this funnel by design, so clearing this bit would only re-ask a question the user already answered | Never cleared in-app (the answer is final); dies with the app container |
...
- **The two append-only ledgers' reset boundary had two residuals, and they are identical for coins
  and milestones** (the milestone marker landed 2026-08-21; the coin one has always worked this way).
  The durability residual is CLOSED since 2026-09-24; its bullet stays below as the record.
  ...
  - **Marker durability — CLOSED 2026-09-24 (tracker §3.6), for both ledgers at once.** Until then a
    failed marker write survived only in the service's in-memory pending queue, so a process death
    before the retry lost the boundary silently. Now the marker is remembered in the store's
    device-local sidecar BEFORE the rows are deleted (`PendingResetBoundaries`; the two keys are
    the kept row "Ledger reset-boundary sidecars" above), every load merges it into the ledger and
    retries its append until it lands, and the sidecar is then retired — so the aggregation and the
    reconciles ignore pre-boundary rows even while the synced marker is missing. The wipe's reported
    verdict still covers the row DELETE only: a boundary is not data left behind. What remains: an
    UNINSTALL before the marker lands loses the sidecar with the app container (the local store goes
    with it, and a reinstall re-imports whatever iCloud holds); and a sidecar write that fails —
    `UserDefaults` offers no failure signal, so in practice only a test stub — degrades that one wipe
    to the pre-2026-09-24 in-memory retry, audited as `resetBoundary.notDurable`.
