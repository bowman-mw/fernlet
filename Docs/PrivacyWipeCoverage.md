...
| Session temp messages — the live-session chat transcript. Dropped in **leg 0b**, at the top of the funnel (behind only the radio stand-down in the row below), through the manager's ONE clear funnel: that is what bumps `transcriptGeneration` **and** what switches the routed projection off for the rest of the wipe. Both halves matter, because the sealed routed store beneath the transcript is not destroyed until leg 11, and every suspension point in between was a window in which a rising access edge re-projected the just-wiped messages into a transcript that is still live, in the same mesh (P6 item 4 fix review). Leg 7b's old direct `sessionMessages.clear()` did neither and is gone. `endPrivacyWipe()` runs from a `defer`, so no exit leaves the projection switched off for the process | Memory (SessionMessageStore) | `meshNetworkManager.beginPrivacyWipe` |
| The four proximity radios — the nearby-friends presence advertiser and its discovery state, the recipe-share listener, the mesh links and the admission door. Until network migration P7 item 3 this row was a bare `presenceManager.stop()` on leg 7b: one radio of four, a third of the way through the funnel, while the mesh kept advertising and admitting peers for the whole wipe. The funnel now raises the run-policy leg at its TOP (leg 0, ahead of the transcript clear), which makes a delete-all one of plan §13's three dominating inputs — `ProximityRunPolicyHost` answers with `stop` on every radio and calls its teardown door (`stopJoin()`, which empties the committed slots and clears the group-key state, plus both listener stops). Lowered from a `defer` registered before `endPrivacyWipe()`'s, so the LIFO order restores the routed projection first and no exit leaves the policy pinned. **Lowering the leg re-arms the radios, and that is deliberate:** a wipe that finishes with the user parked on the Friends tab, foreground and unlocked, has the policy push `run` again the instant the success sheet appears — a fresh `startJoin()` and a fresh give-up clock, with no further user act. It is an improvement rather than a leak, because leg 11 (`wipeIdentityForDeleteAll()`) runs BEFORE that `defer`, so the new search advertises a freshly minted identity; before pass B the mesh advertised on the OLD identity straight through the wipe and after it | PresenceManager, ProximityRecipeShareManager, MeshNetworkManager | `deletingAllDataHook` (the funnel's call site; the host call it is wired to is `setDeletingAllData`, pinned separately through the `ContentView.attachDeleteAllHooks()` half of the scan — an unwired hook is a wipe that no longer stands the radios down) |
