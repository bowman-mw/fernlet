...
/// computed as an overlay (``AIStatusOverlay``) — stored user intent stays as-is; the derived state
/// is `f(stored intent, local counter)`. ``FernletAIGate`` consults it (via the store seam) on every
/// dispatch decision.
public struct AICallQuota: Sendable, Equatable {
    ...
    /// Rehydrates a counter from its persisted parts (the store's decode path).
    ...
    /// Formats `date` as the `yyyy-MM-dd` day key used to detect rollover (see ``dayKeyCalendar``).
...
/// into the *effective* status.
///
/// Off stays off; anything the user left enabled is overlaid with the rate-derived `.sleepy` /
/// `.resting` state. This is the read side the settings label uses instead of the raw stored value,
/// and the derivation ``FernletAIGate`` feeds into ``FernletModelRouter/resolve(tier:effectiveStatus:userInvoked:)``.
/// A caseless enum: it is a pure-function namespace, never instantiated.
public enum AIStatusOverlay {
    /// Derives the effective `AIStatus` at `now` from stored intent plus the local quota; `.off`
    /// always wins, any enabled intent defers to ``AICallQuota/derivedStatus(now:calendar:)``.
...
/// The injectable seam for the device-local counter.
///
/// Declared here so `AIProviders` can consume the quota ONLY through this protocol — the concrete
/// `UserDefaults`-backed store (`UserDefaultsAICallQuotaStore`) lives in the app and is never
/// nameable from the walled module. ``FernletAIGate`` carries a value of this type and charges it on
/// every resolved dispatch. Keeps the counter out of `FernletSnapshot` / sealed stores / CloudKit by
/// construction.
