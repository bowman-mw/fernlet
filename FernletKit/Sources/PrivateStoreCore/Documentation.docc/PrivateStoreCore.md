...
Phase 3 then deleted the **sealed-column** legacy readers too, and the Phase 2.6 migrator with them. `ColumnCrypto` reads V3 and nothing else: the unprefixed no-AAD rung and the `0x02` binding-only rung are gone (owner decision D2 took the second one), and `SealedColumnFormatMigrator` converted *through* both, so with the rungs gone it could heal nothing and was removed rather than left standing. Its latch, its private-hub status capsule and its unlock-tail funding trigger went in the same change, and the two `UserDefaults` wipe rows they owed went with them. Bytes in either retired generation are now refused by name — `ColumnCrypto.SealedColumnOpenError.retiredFormat(_:)`, which says *which* generation the marker reports — while ``SealedColumnFormatCensus`` **stays**: it classifies by marker byte, holds no key, and is what lets a refusal name what it refused. One resolution was genuinely lost and is recorded rather than glossed: the ~1-in-256 legacy blob whose first nonce byte collides with a marker used to be resolved *by open* by the keyed pass, and nothing can resolve it now.
