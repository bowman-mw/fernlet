...
| --- | --- | --- |
| `FernletLockService.reset()` (Settings → Reset app lock) | Destroyed — `KeychainItem.deleteAll(service:)` sweeps every generic password under the lock service, `SecureEnclaveContentKeyWrap.deleteKey` takes the SE wrap outside it | **Fully honest**: crypto-erased *and* the file rebuilt |
