...
The module's most load-bearing idea is the **sanitize barrier**, the data-side analogue of the compiler import wall. The repository write methods do not accept raw values: ``FernletRepository/saveSnapshot(_:)`` requires a ``SanitizedSnapshot``, ``FernletRepository/updateDay(_:for:todayKey:)`` requires a ``SanitizedDay``, and a synced day row is built through ``DayRecordUpsert``'s sanitized mint. Those wrapper types have private initializers, so the only way to obtain one in production is through a `sanitizing` factory that applies the storage privacy strip: sealed-journal text is blanked (today's and previous journals), **every value READ from HealthKit** is removed, and cycle-derived `periodPhase` is stripped from every stored daily score. An un-stripped snapshot therefore cannot reach a potentially iCloud-synced blob *by construction*, regardless of which conformer is active or how a call site is written. The strip itself is factored into one shared per-day helper so the blob path and the per-day path cannot drift.

**HealthKit information is never stored in iCloud** (owner decision 2026-09-23; App Review 5.1.3(ii)). Until then the strip removed only the `cycle`/`intimate` groups and the activity, body and mindfulness readings synced in every day row. The HealthKit half of the strip now lives in `HealthKitStorageStrip.swift` — `FernletDay.strippingHealthKitValues()` drops the whole `healthContext` (so a field added to it later is dropped with it), the sleep hours HealthKit wrote into a user's sleep log (recognized by value against the context's `healthKitSleepLogHours` marker or its `body.sleepHours`; a typed value that differs is the user's and stays, as do their rating and note), and every workout imported from Apple Health (Fernlet-logged workouts, authored ones included, are the user's). `storedDailyScores` also drops the scores' `healthActivityContext`/`healthBodyContext`. Nothing the strip removes is lost on the device that read it: ``DeviceHealthResidue`` is the exact complement of the strip, `FernletDay.healthKitResidue` extracts it, `overlayingHealthKitResidue(_:)` puts it back on read, and ``DeviceHealthResidueStoring`` is the contract for the device-local, non-synced, backup-excluded, bounded cache that holds it — implemented in the app target (never in a module the walled `CloudKitSync` imports) and reached by `DiaryStore` only through this protocol. Other devices derive their own context from their own HealthKit store. The same cache holds ``DeviceHealthBodyProfile`` — the age, sex, height and weight this device imported from HealthKit, which until the 2026-09-23 follow-up overwrote the synced `settings.userProfile` (`DiaryStore.effectiveUserProfile` lays it over the typed profile; `typedProfile(_:editedFrom:to:)` keeps a profile-screen edit from carrying a merely-shown Health value into settings). The composite daily score and its component sub-scores are the app's own index and still sync; they are computed partly from HealthKit inputs (see the 2026-09-23 round report for the owner question this leaves).

That barrier is also this module's relationship to the S3 privacy wall. In the package graph (`FernletKit/Package.swift`), FernletPersistence sits at Layer 2 with a single in-package dependency, `FernletDomainModel` — and it is itself a declared dependency of `CloudKitSync`, one of the two walled consumers. So this module is *outside* the wall: everything it defines is nameable by sync code, and nothing in it may reference the sealed `Private*` stores (it doesn't, and the `DIAGNOSE_MISSING_TARGET_DEPENDENCIES=YES_ERROR` enforcement would make a forbidden import a hard build error). The wall keeps sync code from importing sealed *types*; the sanitize barrier keeps sealed *data* — journal plaintext — and HealthKit data out of the values sync code is handed. The two mechanisms are complementary halves of the same privacy guarantee. Sealing state itself (which journal IDs are sealed) lives app-side and is passed into the mints as plain data.

The five per-row contracts encode the multi-device merge strategy. The snapshot blob is effectively last-writer-wins, so anything that must survive two devices writing concurrently is carved into its own row store whose contract is **append/upsert-only**: `upsert`/`append` touch only the rows they are handed and never delete rows they didn't receive, so rows union-merge across devices and one device can never clobber another's synced rows. Each contract carries a deliberate variation: ``DayRecordRepositoring`` splits the day history into per-day rows (which is what allowed the old 370-day blob cap to be removed), ``MilestoneLedgerRepositoring`` still has no delete API on the protocol — the milestone row delete (`deleteAll()`, added 2026-08-20 when the wipe stopped keeping the milestone trail, reversing the earlier survive-a-reset rule) lives only on the concrete CloudKitSync conformer, reachable only by the app's deletion funnel — and the others (`deleteAll`) reserve whole-store deletion for a full account reset. Several of these replaced earlier full-replace `save(_:)` APIs that carried latent cross-device clobber bugs — the contracts now make the safe behavior structural. The two ledger contracts additionally carry a device-local, never-synced sidecar for the wipe's **pending reset boundary** (`pendingResetBoundaries()` / `savePendingResetBoundaries(_:)`, 2026-09-24): the marker a reset appends is recorded there before any row is deleted, merged into every load and retried until its synced append lands, then retired — so a process death between a failed marker append and its retry can no longer let another device's rows count again. A sidecar is not a row delete, so ``MilestoneLedgerRepositoring`` still carries none.
...
### HealthKit values on this device only

- ``DeviceHealthResidue``
- ``DeviceHealthResidueStoring``
- ``DeviceHealthBodyProfile``
