...
//
//   * `applyRoutedAccessGate(` appears EXACTLY ONCE outside ProximityKit. Before this item the app
//     assembled the gate in `FernletApp.pushRoutedAccessGate(_:protectedData:foreground:)` and
//     called the door from six sites (`FernletApp.swift:337`, `:383`, `:416`, `:435`, `:447`,
//     `:491`); four of them had compared `== .active` while the scene handler fell only on
//     `.background`, which is the failure a single writer exists to make impossible.
...
// claim the others are instances of: whatever the host writes equals
// `ProximityRunPolicy.decide(host.inputs).accessGate` for the inputs it held at that moment.
...
@Suite struct ProximityRunPolicyHostTests {
    ...
    /// One leg update per entry, covering every setter the host declares.
    ///
    ...
    /// records its leg and re-decides. The order deliberately moves a leg back and forth (the scene
    /// three times, protected data twice, the lock in and out of duress) so a setter that assigned
    /// the wrong field would be caught by the general cell below rather than hidden by a monotone
    /// sequence.
    static let legSteps: [@MainActor (ProximityRunPolicyHost) -> Void] = [
        { $0.setScenePhase(.background) },
        { $0.setProtectedDataAvailable(true) },
        { $0.setScenePhase(.inactive) },
        { $0.setAppLockState(.duress) },
        { $0.setSelectedTab(.social) },
        { $0.setHasCommittedPeer(true) },
        { $0.setAppLockState(.unlocked) },
        { $0.setChatAgeGate(.below) },
        { $0.setAllowsNearbyPresence(true) },
        { $0.setAllowsNearbyRecipeShares(true) },
        { $0.setDeletingAllData(true) },
        { $0.setScenePhase(.active) },
        { $0.setProtectedDataAvailable(false) }
    ]
    ...
    @Test func theRoutedAccessGateHasExactlyOneWriterInTheAppTarget() throws {
        ...
        for source in sources {
            if Self.occurrences(of: "applyRoutedAccessGate(", in: source.code) > 0 {
                writers.append(source.name)
            }
            if Self.occurrences(of: "ProximityRunPolicy.decide(", in: source.code) > 0 {
    ...
    @Test func anInactiveSceneLegPushesAForegroundGate() throws {
        ...
        #expect(gate == ProximityRunPolicy.decide(host.inputs).accessGate,
                "the written gate is the policy's answer for the inputs the host held")
    }
    ...
    /// The general claim the five cells above are instances of: whatever the host writes is exactly
    /// `ProximityRunPolicy.decide(inputs).accessGate` for the inputs it held at that moment, over a
    /// sequence that exercises every setter the host declares.
    ///
    ...
    /// rather than one per CHANGED leg update.
    @Test func everyWrittenGateIsThePolicysDecisionForTheSameInputs() {
        let (host, recorder) = Self.connectedHost()
        var drift: [Int] = []
        var index = 0
        // R2: bounded by the literal step list.
        for step in Self.legSteps {
            step(host)
            if recorder.gates.last != ProximityRunPolicy.decide(host.inputs).accessGate {
                drift.append(index)
            }
            index += 1
        }
        ...
        #expect(drift.isEmpty,
                "a written gate drifted from the policy's decision for the same inputs")
