...
> **As-built note (2026-06, `PeriodContextBridge`).** The shipped bridge is intentionally *stricter* than the paragraph above. `PeriodTrackerStore.loadEntries` nils the `CyclePrediction` whenever the content key is absent (locked), so while locked the bridge resolves **only** `.menstrual`/`.unknown` from observed flow — the calendar-math phases (follicular/ovulatory/luteal), the nutrition/exercise hints, *and* scoring softening all stay off until unlock. This was a deliberate product call: the lock is treated as a "forget the whole cycle" gate, not merely a notes gate, which keeps the companion/score from reflecting cycle phase while the section is locked. It costs nothing in practice because, as the paragraph notes, the live callers only consult the bridge from already-unlocked contexts. Deriving an HK-only prediction while locked would leak nothing beyond the OS-readable flow data (`predict`/`detectPeriods` never touch sealed narratives), so the literal §5.3 behavior remains a safe future option if a locked caller is ever added — flip the `loadEntries` gate and keep the `unlocked`-gated nutrition/exercise/trend paths as-is.

> **Caching exception (2026-06).** Despite this section's title, the bridge keeps exactly one memo — `cachedPeriodStarts`, the detected period-start dates — because period-aware scoring reads `score` ~12×/render and each read would otherwise re-group up to 240 days of flow via `detectPeriods`. It is a pure derived value, recomputed from the live `entries` and invalidated in `refresh()` on every mutation (launch / lock-change / delete / log-edit), and the observed-flow check never consults it. So "recompute, don't cache" still holds at the signal level: a wiped entry resolves to `.unknown` on the very next read.
