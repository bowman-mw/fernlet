...
``FernletApp`` is the `@main` entry point and launch state machine. It owns the app-lifetime singletons a cold launch needs before any view exists — the keychain-backed `FernletLockService`, the `StoragePreferencesStore`, and ``FernletStoreLoader``, which loads the store off the first frame and drives the preparing/ready/failed launch phases. Scene-phase changes relock the app and flush pending saves; storage-preference changes reload persistence and re-apply the sealed store's backup exclusion.
