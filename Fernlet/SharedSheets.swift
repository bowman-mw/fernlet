...
struct TextureSheet: View {
        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    SheetField("Observation") {
                        SheetTextEditor(text: $text, placeholder: "How did something feel?", minHeight: 120)
                    }

                    SheetField("Tags") {
                        FlowLayout(spacing: 8) {
                            ForEach(TextureTag.allCases) { tag in
                                Button(tag.rawValue.capitalized) { toggle(tag) }
                                    .buttonStyle(ChipButtonStyle(selected: tags.contains(tag)))
                            }
                        }
                    }
                }
            ...
            SheetSaveBar(disabled: text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) {
                store.addTexture(text, tags: tags)
                dismiss()
            }
        }
        .background(Color.parchment)
    ...
    private func toggle(_ tag: TextureTag) {
