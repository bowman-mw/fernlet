...
struct FernletTests {
    ...
    @Test func addWorkoutDispatchesHealthKitSaveTask() async {
        ...
        let authorizedStore = FernletStore(repository: LocalFernletRepository(fileURL: temporaryDatabaseURL("workout-hk-save-authorized")), healthKitService: authorizedService)
        ...
        let unauthorizedStore = FernletStore(repository: LocalFernletRepository(fileURL: temporaryDatabaseURL("workout-hk-save-unauthorized")), healthKitService: unauthorizedService)
    ...
    @Test func addWorkoutPersistsHealthKitUUIDOnSuccess() async throws {
        ...
        let store = FernletStore(repository: LocalFernletRepository(fileURL: temporaryDatabaseURL("workout-hk-uuid")), healthKitService: service)
    ...
    @Test func personalCareTasksCanBeCustomizedAndPersisted() throws {
        ...
        let loadedStore = FernletStore(date: Date(), repository: repository)
    ...
    @Test func storeUsesStoredDailyHealthScoreSummary() throws {
        ...
        let store = FernletStore(date: computedAt, repository: repository)
